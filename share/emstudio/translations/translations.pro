...
TRANSLATIONS = $$prependAll(LANGUAGES, $$PWD/emstudio_,.ts)
...
for(dir, $$list($$files($$IDE_SOURCE_TREE/share/emstudio/templates/wizards/*, true))):CUSTOMWIZARD_FILES += $$files($$dir/wizard.xml)
CUSTOMWIZARD_FILES = \"$$join(CUSTOMWIZARD_FILES, "|$$PREFIX", "$$PREFIX")\"

for(file, $$list($$files($$IDE_SOURCE_TREE/src/share/emstudio/externaltools/*.xml))):EXTERNALTOOLS_FILES += $$files($$file)
EXTERNALTOOLS_FILES = \"$$join(EXTERNALTOOLS_FILES, "|$$PREFIX", "$$PREFIX")\"

for(file, $$list($$files($$IDE_SOURCE_TREE/share/emstudio/snippets/*))):SNIPPETS_FILES += $$files($$file)
...
extract.commands += \
    ...
    $(QMAKE) -o Makefile.jsonwizard JSONWIZARD_TR_H=\"$$JSONWIZARD_TR_H\" TOP_LEVEL=\"$$IDE_SOURCE_TREE/share/emstudio/templates/wizards\" $$PWD/jsonwizard_tr.pro
...
sources = src/app src/libs $$plugin_sources $$shared_sources share/emstudio/qmldesigner

for(path, INCLUDEPATH): include_options *= -I$$shell_quote($$path)

files = $$files($$PWD/*_??.ts) $$PWD/emstudio_untranslated.ts
for(file, files) {
...
isEqual(QMAKE_DIR_SEP, /) {
    commit-ts.commands = \
        cd $$wd; \
        git add -N share/emstudio/translations/*_??.ts && \
        for f in `git diff-files --name-only share/emstudio/translations/*_??.ts`; do \
            $$LCONVERT -locations none -i \$\$f -o \$\$f; \
        done; \
        git add share/emstudio/translations/*_??.ts && git commit
} else {
    commit-ts.commands = \
        cd $$wd && \
        git add -N share/emstudio/translations/*_??.ts && \
        for /f usebackq %%f in (`git diff-files --name-only share/emstudio/translations/*_??.ts`) do \
            $$LCONVERT -locations none -i %%f -o %%f $$escape_expand(\\n\\t) \
        cd $$wd && git add share/emstudio/translations/*_??.ts && git commit
}
...
qmfiles.files = $$prependAll(LANGUAGES, $$IDE_DATA_PATH/translations/emstudio_,.qm)
