!isEmpty(EMSTUDIO_PRI_INCLUDED):error("emos_x.pri already included")
EMSTUDIO_PRI_INCLUDED = 1

include($$PWD/emos_ide_branding.pri)
!isEmpty(IDE_BRANDING_PRI): include($$IDE_BRANDING_PRI)

PRODUCT_BUNDLE_IDENTIFIER=$${PRODUCT_BUNDLE_ORGANIZATION}.$${IDE_ID}
#VERSION = $$EMSTUDIO_VERSION #不增加版本到dll名
...
defineReplace(qtLibraryName) {
   ...
   #不增加版本到dll名
#   win32 {
#      VERSION_LIST = $$split(EMSTUDIO_VERSION, .)
...
osx {
    ...
    IDE_LIBRARY_PATH = $$IDE_OUTPUT_PATH/$$IDE_LIBRARY_BASENAME/emstudio
    IDE_PLUGIN_PATH  = $$IDE_LIBRARY_PATH/plugins
    IDE_DATA_PATH    = $$IDE_OUTPUT_PATH/share/emstudio
    IDE_DOC_PATH     = $$IDE_OUTPUT_PATH/share/doc/emstudio
    IDE_BIN_PATH     = $$IDE_OUTPUT_PATH/bin
    win32: \
        IDE_LIBEXEC_PATH = $$IDE_OUTPUT_PATH/bin
    else: \
        IDE_LIBEXEC_PATH = $$IDE_OUTPUT_PATH/libexec/emstudio
    !isEqual(IDE_SOURCE_TREE, $$IDE_OUTPUT_PATH):copydata = 1

    LINK_LIBRARY_PATH = $$IDE_BUILD_TREE/$$IDE_LIBRARY_BASENAME/emstudio
    LINK_PLUGIN_PATH  = $$LINK_LIBRARY_PATH/plugins

    INSTALL_LIBRARY_PATH = $$QTC_PREFIX/$$IDE_LIBRARY_BASENAME/emstudio
    INSTALL_PLUGIN_PATH  = $$INSTALL_LIBRARY_PATH/plugins
    win32: \
        INSTALL_LIBEXEC_PATH = $$QTC_PREFIX/bin
    else: \
        INSTALL_LIBEXEC_PATH = $$QTC_PREFIX/libexec/emstudio
    INSTALL_DATA_PATH    = $$QTC_PREFIX/share/emstudio
    INSTALL_DOC_PATH     = $$QTC_PREFIX/share/doc/emstudio
...
win32:exists($$IDE_BUILD_TREE/lib/emstudio) {
    # for .lib in case of binary package with dev package
    LIBS *= -L$$IDE_BUILD_TREE/lib/emstudio
    LIBS *= -L$$IDE_BUILD_TREE/lib/emstudio/plugins
}
...
QTC_LIB_DIRS += $$IDE_SOURCE_TREE/src/libs/thirdparty
