...
void tst_Algorithm::count()
{
    {
        const QList<QString> strings({"1", "3", "132"});
        QVERIFY(1 == Utils::count(strings, [](const QString &s) { return s == "132"; }));
    }
//    {
//        const QList<int> list({0, 0, 0, 0, 1, 0, 0});
//        QVERIFY(6 == Utils::count(list, 0));
//    }
}

void tst_Algorithm::allOf()
{
    {
        const QList<Struct> list({2, 4, 6, 8});
        QVERIFY(Utils::allOf(list, &Struct::isEven));
    }
    {
        const QList<Struct> list({0, 0, 0, 0, 1, 0, 0});
        QVERIFY(!Utils::allOf(list, &Struct::member));
    }
    {
        const QList<Struct> list({1, 1, 1, 1, 1, 1, 1});
        QVERIFY(Utils::allOf(list, &Struct::member));
    }

}

void tst_Algorithm::erase()
{
    {
        QList<int> list({2, 4, 6, 8});
        Utils::erase(list, [](int i){ return i == 2; });
        QVERIFY(list.first() == 4);
    }
}

void tst_Algorithm::maxElement()
{
    {
        QList<int> list({2, 4, 6, 8});
        QVERIFY(Utils::maxElementOr(list, 0) == 8);
    }
//    {
//        QList<Struct> list({2, 4, 6, 8});
//        QVERIFY(Utils::maxElementOr(list, Struct(0)) == Struct(8));
//    }
    {
        QList<int> list({2, 2, 2, 2});
        QVERIFY(Utils::maxElementOr(list, 0) == 2);
    }
    {
        QList<int> list({});
        QVERIFY(Utils::maxElementOr(list, 0) == 0);
    }
}

void tst_Algorithm::filter()
{
    {
        const QList<QString> strings({"1", "3", "132"});
        QVERIFY(QString("132") == Utils::filtered(strings, [](const QString &s) { return s == "132"; }).first());
    }
    {
        const QList<QString> strings({"1", "3", "132", "3"});
       QList<QString> ss = Utils::filteredUnique(strings);

        QVERIFY(Utils::filteredUnique(strings).size() == 3);
    }
    {
        const QList<QString> strings({"1", "3", "132"});

        QList<QString> partition1;
        QList<QString> partition2;
        std::tie(partition1, partition2) = Utils::partition(strings, [](const QString &s) { return s == "132"; });
        QVERIFY(QString("132") == partition1.first());
        QVERIFY(QString("1") == partition2.first());
        QVERIFY(QString("3") == partition2.last());
    }
}
...
void tst_Algorithm::indexOf()
{
    std::vector<int> v1{1, 2, 3, 4};
    QVERIFY(1 == Utils::indexOf(v1, [](int i) { return i == 2; }));
    QList<Struct> structs = {2, 4, 6, 8};
    QVERIFY(0 == Utils::indexOf(structs, [](Struct i) { return i.member % 2 == 0; }));
    QVERIFY(-1 == Utils::indexOf(structs, [](Struct i) { return i.member % 2 == 1; }));
}
