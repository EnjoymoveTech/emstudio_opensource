...
{
    Context() {}
    const QString emStudioElement = QString("emstudio");
...
private:
    enum Element { EmStudioElement, DataElement, VariableElement,
...
ParseContext::Element ParseContext::element(const StringView &r) const
{
    ...
    if (r == emStudioElement)
        return EmStudioElement;
    if (r == dataElement)
        return DataElement;
...
bool PersistentSettingsWriter::write(const QVariantMap &data, QString *errorString) const
{
    ...
    if (!saver.hasError()) {
        ...
        w.writeStartElement(ctx.emStudioElement);
