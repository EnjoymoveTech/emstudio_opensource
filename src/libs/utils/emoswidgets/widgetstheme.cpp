...
{
    Q_DISABLE_COPY(WidgetsThemePrivate)
    Q_DECLARE_PUBLIC(WidgetsTheme)

public:
    WidgetsThemePrivate(WidgetsTheme *q);
    ~WidgetsThemePrivate();

    WidgetsTheme *const q_ptr;
    QHash<QString, QColor> colors;
};

WidgetsThemePrivate::WidgetsThemePrivate(WidgetsTheme *q)
    : q_ptr(q)
{
}

WidgetsThemePrivate::~WidgetsThemePrivate()
{
}
...
WidgetsTheme::WidgetsTheme(QObject *parent)
    : QObject(parent),
      d_ptr(new WidgetsThemePrivate(this))
{
    setColor("primary1", grey300);//主色调
    setColor("primary2", grey100);//亮一级
    setColor("primary3", grey500);//暗一级
    setColor("primary4", lightBlack);
    setColor("accent1", green500);//强调色
    setColor("accent2", greenA700);
    setColor("accent3", green700);
    setColor("text", darkBlack);
    setColor("alternateText", white);
    setColor("canvas", white);
    setColor("border", grey300);
    setColor("disabled", minBlack);
    setColor("disabled2", faintBlack);
    setColor("disabled3", grey300);
}
...
QColor WidgetsTheme::getColor(const QString &key) const
{
    Q_D(const WidgetsTheme);
...
QColor WidgetsTheme::getColor(MaterialColor color) const
{
    return palette[color];
}

void WidgetsTheme::setColor(const QString &key, const QColor &color)
{
    Q_D(WidgetsTheme);
...
void WidgetsTheme::setColor(const QString &key, MaterialColor color)
{
    Q_D(WidgetsTheme);
...
QIcon WidgetsTheme::icon(QString category, QString icon, IconSize size)
{
    return QIcon(":/icons/icons/" % category % "/svg/production/ic_" % icon % "_24px.svg");
}

QString WidgetsTheme::converARGB16Hex(QColor color)
{
    ...
    QString hexStr = "#"+alphaStr+redStr+greenStr+blueStr;
    //返回hexStr
    return hexStr;
}
}
