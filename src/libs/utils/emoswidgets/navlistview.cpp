...
protected:
    virtual bool filterAcceptsRow(int source_row, const QModelIndex &source_parent) const override{
        if (!source_parent.isValid()) {
            ...
            NavModel::TreeNode *node = (NavModel::TreeNode *)categoryIndex.data(Qt::UserRole).toULongLong();
...
NavDelegate::NavDelegate(QObject *parent) : QStyledItemDelegate(parent)
{
    ...
    if (!fontDb.families().contains("FontAwesome")) {
        int fontId = fontDb.addApplicationFont(":/fonts/webfont_fontawesome");
...
QSize NavDelegate::sizeHint(const QStyleOptionViewItem &option, const QModelIndex &index) const
{
    ...
    QSize size(50, nav->getParentHeight());
    return size;
}
...
void NavDelegate::paint(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const
{
    ...
    if (option.state & QStyle::State_Selected) {
        ...
        iconColor = parent ? nav->getParentTextSelectedColor() : nav->getChildIconColor();
        ...
        tipBgColor = parent ? nav->getParentTextSelectedColor() : nav->getChildTextSelectedColor();
        tipTextColor = parent ? nav->getParentBgSelectedColor() : nav->getChildBgSelectedColor();
        iconColor = parent ? nav->getParentTextHoverColor() : nav->getChildIconColor();
    } else {
        ...
        iconColor = parent ? nav->getParentTextNormalColor() : nav->getChildIconColor();
    }
    ...
    if (nav->getTipVisible() && !tip.isEmpty()) {
        ...
        tipRect.setLeft(optionRect.right() - nav->getTipWidth() - 4);
        tipRect.setRight(optionRect.right() - 4);
...
void NavModel::showList(bool expand)
{
    for(int i = 0; i < listNode.size(); i++)
    {
        TreeNode *node = listNode[i].treeNode;
        ...
        if (!node->expand) {
            beginInsertRows(index(i), 1, node->children.size());
            endInsertRows();
        } else {
            beginRemoveRows(index(i), 1, 1);
            endRemoveRows();
        }
    }
}
...
void NavModel::expand(const QModelIndex &index)
{
    ...
    if (!node->expand) {
        beginInsertRows(index, 0, node->children.size());
        endInsertRows();
    } else {
...
void NavModel::refreshList()
{
    ...
    for (QList<TreeNode *>::iterator it = treeNode.begin(); it != treeNode.end(); ++it) {
        ...
        for (QList<TreeNode *>::iterator child = (*it)->children.begin(); child != (*it)->children.end(); ++child) {
            if(m_strFilter != "")
            {
                QRegExp regxName(m_strFilter);
                regxName.setCaseSensitivity(Qt::CaseInsensitive);
                if (regxName.indexIn((*child)->text) == -1)//未匹配
                    continue;
            }
            ListNode node;
...
void NavModel::setFilter(const QString &filter)
{
    m_strFilter = filter;
    refreshList();
    beginResetModel();
...
QColor NavListView::getChildIconColor() const
{
...
void NavListView::setChildIconColor(const QColor &childIconColor)
{
    this->childIconColor = childIconColor;
}
...
void NavListView::showList(bool expand)
{
    model->showList(expand);
}

void NavListView::setFilterReg(const QString &text)
{
