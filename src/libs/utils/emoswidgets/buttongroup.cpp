...
ButtonGroup::ButtonGroup(QWidget *parent) : QWidget(parent)
{
    ...
    QColor text = WidgetsStyle::instance().themeColor(DefC_Text);
    ...
    textSize = 15;
    texts = "";
    linePosition = LinePosition_Bottom;

    baseColor = primary1;
    btnNormalColor = primary1;
    lineColor = accent3;
    accent1.setAlpha(180);
    btnHoverColor = accent1;
    accent1.setAlpha(255);
    btnDarkColor = accent1;

    textHoverColor = alternateText;
    textDarkColor = alternateText;
    textNormalColor = text;
    ...
    setStyle(&WidgetsStyle::instance());
    QFont font("Roboto");
...
LinePosition ButtonGroup::getLinePosition() const
{
...
void ButtonGroup::setLinePosition(const LinePosition &linePosition)
{
...
void ButtonGroup::init(bool click)
{
    //首次显示重新纠正按钮尺寸大小
    if (btns.count() > 0) {
