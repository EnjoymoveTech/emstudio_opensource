...
class EMSTUDIO_UTILS_EXPORT NavListView : public QListView
{
    Q_OBJECT
    Q_ENUMS(ExpendMode)
    ...
    Q_PROPERTY(QColor childIconColor READ getChildIconColor WRITE setChildIconColor)
...
private:
    QSortFilterProxyModel *m_sortModel; //筛选模型;
    NavModel *model;                //数据模型
...
public:
    ...
    QColor getChildIconColor()      const;
...
public Q_SLOTS:
    ...
    void setChildIconColor(const QColor &childIconColor);
    ...
    //筛选
    void setFilterReg(const QString& text);
Q_SIGNALS:
