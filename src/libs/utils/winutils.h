...
EMSTUDIO_UTILS_EXPORT QString winErrorMessage(unsigned long error);

// Determine a DLL version
enum WinDLLVersionType { WinDLLFileVersion, WinDLLProductVersion };
EMSTUDIO_UTILS_EXPORT QString winGetDLLVersion(WinDLLVersionType t,
                                                const QString &name,
                                                QString *errorMessage);

EMSTUDIO_UTILS_EXPORT bool is64BitWindowsSystem();

// Check for a 64bit binary.
EMSTUDIO_UTILS_EXPORT bool is64BitWindowsBinary(const QString &binary);

// Get the path to the executable for a given PID.
EMSTUDIO_UTILS_EXPORT QString imageName(quint32 processId);
...
//
class EMSTUDIO_UTILS_EXPORT WindowsCrashDialogBlocker {
public:
