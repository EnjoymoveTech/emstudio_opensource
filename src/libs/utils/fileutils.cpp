...
XmlFileSaver::XmlFileSaver(const QString &filename, QIODevice::OpenMode mode) : FileSaver(filename, mode)
{

}

void XmlFileSaver::saveNodeCanonically(QXmlStreamWriter *stream, const QDomNode &domNode)
{
    if (stream->hasError())
    {
        return;
    ...
    if (domNode.isElement())
    {
        ...
        if (!domElement.isNull())
        {
            ...
            if (domElement.hasAttributes())
            {
                ...
                QList<QString> attributeKeysList = attributes.keys();
                std::stable_sort(attributeKeysList.begin(),attributeKeysList.end());
                foreach (const QString & key,attributeKeysList)
                {
                    stream->writeAttribute(key, attributes[key]);
                }
            }
...
bool XmlFileSaver::write(QDomDocument *doc)
{
    if (m_hasError)
        return false;

    std::unique_ptr<QXmlStreamWriter> writer { new QXmlStreamWriter(m_file.get()) };
    ...
    while (!root.isNull())
    {
        saveNodeCanonically(writer.get(), root);
...
QString XmlFileSaver::serializeStr(QDomDocument *doc)
{
    QString xmlString;
    std::unique_ptr<QXmlStreamWriter> writer { new QXmlStreamWriter(&xmlString) };
    writer->setAutoFormatting(true);
    if (!doc)
    {
        return QString();
...
QString XmlFileSaver::serializeStr(const QString &content)
{
    QDomDocument doc;
    doc.setContent(content);
    return serializeStr(&doc);
}
