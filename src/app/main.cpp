...
****************************************************************************/
#if 0
#include "../tools/emstudiocrashhandler/crashhandlersetup.h"
#endif
#include <app/app_version.h>

#include <pluginmanager/iplugin.h>
#include <pluginmanager/pluginerroroverview.h>
...
const char USER_LIBRARY_PATH_OPTION[] = "-user-library-path"; // hidden option for emstudio.sh
...
static inline QStringList getPluginPaths()
{
    ...
    //    "%LOCALAPPDATA%\EmStudio\emstudio" on Windows Vista and later
    //    "$XDG_DATA_HOME/data/EmStudio/emstudio" or "~/.local/share/data/EmStudio/emstudio" on Linux
    //    "~/Library/Application Support/EmStudio/Qt Creator" on Mac
...
int main(int argc, char **argv)
{
    ...
    if (qEnvironmentVariableIsSet("EMSTUDIO_DISABLE_NATIVE_MENUBAR")
    ...
    PluginManager::setPluginIID(QLatin1String("org.qt-project.Qt.EmStudioPlugin"));
    ...
    if (uiLanguages.contains("en-US"))
    {
        uiLanguages = QStringList("en-US");
    }
    ...
    for (QString locale : qAsConst(uiLanguages)) {
        ...
        if (translator.load("emstudio_" + locale, creatorTrPath)) {
    ...
    app.setDesktopFileName("org.qt-project.emstudio.desktop");
