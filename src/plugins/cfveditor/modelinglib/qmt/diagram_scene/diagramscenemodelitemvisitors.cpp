...
void DiagramSceneModel::CreationVisitor::visitDDevice(DDevice *device)
{
    QMT_CHECK(!m_graphicsItem);
    m_graphicsItem = new DeviceItem(device, m_diagramSceneModel);
}
...
void DiagramSceneModel::UpdateVisitor::visitDDevice(DDevice *device)
{
    Q_UNUSED(device) // avoid warning in release mode
    QMT_ASSERT(m_graphicsItem, return);

    DeviceItem *deviceItem = qgraphicsitem_cast<DeviceItem *>(m_graphicsItem);
    QMT_ASSERT(deviceItem, return);
    QMT_CHECK(deviceItem->boundary() == device);
    deviceItem->update();
}
