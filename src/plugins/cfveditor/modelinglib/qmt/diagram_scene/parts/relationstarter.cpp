...
{
    //cfv连线的框的底部和边框颜色设置
    setBrush(QBrush(QColor(245,245,220)));
    setPen(QPen(QColor(210,180,140)));
                               ...
                               const QString &toolTip)
{
    ...
    //haojie.fang add style
    if(id == "Method")
        arrow->update(m_diagramSceneModel->styleController()->methodStarterStyle());
    else if(id == "Event")
        arrow->update(m_diagramSceneModel->styleController()->eventStarterStyle());
    else if(id == "Field")
        arrow->update(m_diagramSceneModel->styleController()->fieldStarterStyle());
    else if(id == "Pin")
        arrow->update(m_diagramSceneModel->styleController()->pinStarterStyle());
    else
...
void RelationStarter::mousePressEvent(QGraphicsSceneMouseEvent *event)
{
    ...
    foreach (ArrowItem *item, m_arrows) {
        if (item->boundingRect().contains(mapToItem(item, event->pos()))) {
            ...
            //haojie.fang add style
            if(m_arrowIds.value(item) == "Method")
                m_currentStyle = const_cast<Style*>(m_diagramSceneModel->styleController()->methodStarterStyle());
            else if(m_arrowIds.value(item) == "Event")
                m_currentStyle = const_cast<Style*>(m_diagramSceneModel->styleController()->eventStarterStyle());
            else if(m_arrowIds.value(item) == "Field")
                m_currentStyle = const_cast<Style*>(m_diagramSceneModel->styleController()->fieldStarterStyle());
            else if(m_arrowIds.value(item) == "Pin")
                m_currentStyle = const_cast<Style*>(m_diagramSceneModel->styleController()->pinStarterStyle());
            else
                m_currentStyle = const_cast<Style*>(m_diagramSceneModel->styleController()->relationStarterStyle());
            m_currentPreviewArrow->update(m_currentStyle);
