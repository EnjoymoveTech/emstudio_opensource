...
QLineF AssociationItem::directLineForType(const QPointF& point1, const QPointF& point2)
{
    QLineF directLine;
    directLine.setP1(point1);

    int distance = 30;
    if(m_association->endA().kind() == MAssociationEnd::Event)
    {
        QPointF point2_re = QPointF(point2.x() - distance, point2.y() - distance);
        if((point1.x() > point2.x() && point1.y() > point2.y()) || //二四象限
            (point1.x() < point2.x() && point1.y() < point2.y()))
            point2_re = QPointF(point2.x() + distance, point2.y() - distance);
        else
            point2_re = QPointF(point2.x() - distance, point2.y() - distance);
        directLine.setP2(point2_re);
    }
    else if(m_association->endA().kind() == MAssociationEnd::Field)
    {
        QPointF point2_re = QPointF(point2.x() + distance, point2.y() + distance);
        if((point1.x() > point2.x() && point1.y() > point2.y()) || //一三象限
            (point1.x() < point2.x() && point1.y() < point2.y()))
            point2_re = QPointF(point2.x() - distance, point2.y() + distance);
        else
            point2_re = QPointF(point2.x() + distance, point2.y() + distance);
    ...
    else if(m_association->endA().kind() == MAssociationEnd::Pin)
    {
        QPointF point2_re = QPointF(point2.x() + distance / 2, point2.y() + distance / 2);
        if((point1.x() > point2.x() && point1.y() > point2.y()) || //一三象限
            (point1.x() < point2.x() && point1.y() < point2.y()))
            point2_re = QPointF(point2.x() - distance / 2, point2.y() + distance / 2);
        else
            point2_re = QPointF(point2.x() + distance / 2, point2.y() + distance / 2);
        directLine.setP2(point2_re);
    }
