...
static const qreal MINIMUM_INNER_WIDTH = 120.0;
static const qreal MINIMUM_INNER_HEIGHT = 150.0;
...
void BoundaryItem::update()
{
    ...
    QPen borderPen(QBrush(Qt::black), 1, Qt::DashLine);
...
//    else
//        setZValue(-1 * BOUNDARY_ITEMS_ZVALUE);
    ...
    if (event->button() == Qt::LeftButton || event->button() == Qt::RightButton) {
        ...
        if (!collidingItems.isEmpty()) {
            for (QGraphicsItem *item : collidingItems) {
                if (item != this
                        && m_diagramSceneModel->isInFrontOf(this, item)// haojie.fang
...
void BoundaryItem::contextMenuEvent(QGraphicsSceneContextMenuEvent *event)
{
    QMenu menu;
    menu.addAction(
