...
public:
    ...
    {//haojie.fang连线形状设置
        ...
        case MAssociationEnd::Method:
            //m_arrow->setShaft(ArrowItem::ShaftDashed);//method 画虚线
            endAHead = aNav ? ArrowItem::HeadTriangle : ArrowItem::HeadTriangle;
            break;
        case MAssociationEnd::Event:
            endAHead = aNav ? ArrowItem::HeadTriangle : ArrowItem::HeadNone;
            break;
        ...
        case MAssociationEnd::Method:
            //m_arrow->setShaft(ArrowItem::ShaftDashed);
            endBHead = bNav ? ArrowItem::HeadNone : ArrowItem::HeadNone;//haojie.fang 释放线的形状
            break;
        case MAssociationEnd::Event:
            endBHead = bNav ? ArrowItem::HeadTriangle : ArrowItem::HeadNone;
            break;
        case MAssociationEnd::Field:
            endBHead = bNav ? ArrowItem::HeadTriangle : ArrowItem::HeadNone;
            break;
...
void RelationItem::update(const Style *style)
{
    ...
    if (isSelected() || isSecondarySelected()) {
        ...
        //haojie.fang 默认添加一个锚点
//        if(m_selectionHandles->points().size() == 2)
//            insertHandle(1, (points[0] + points[1]) / 2 + this->pos(), 5, 5);
...
const Style *RelationItem::adaptedStyle()
{
    ...
    //haojie.fang add custom relation color
    Style* style = const_cast<Style*>(m_diagramSceneModel->styleController()->adaptRelationStyle(styledRelation));
    MAssociationEnd::Kind kA = reinterpret_cast<DAssociation*>(m_relation)->endA().kind();
    MAssociationEnd::Kind kB = reinterpret_cast<DAssociation*>(m_relation)->endB().kind();
    if(kA == MAssociationEnd::Method || kB == MAssociationEnd::Method)
        style->setLinePen(QPen(QColor(255,99,71)));
    else if(kA == MAssociationEnd::Event || kB == MAssociationEnd::Event)
        style->setLinePen(QPen(QColor(72,118,255)));
    else if(kA == MAssociationEnd::Field || kB == MAssociationEnd::Field)
        style->setLinePen(QPen(QColor(0,250,154)));
    else if(kA == MAssociationEnd::Pin || kB == MAssociationEnd::Pin)
        style->setLinePen(QPen(QColor(0,0,0)));
...
QPointF RelationItem::calcEndPoint(const Uid &end, const QPointF &otherEndPos, int nearestIntermediatePointIndex)
{
    ...
    if (endObjectItem) {
        ...
        QLineF directLine = directLineForType(endObject->pos(), otherPos);//haojie.fang 不同类型线不能重合
...
QLineF RelationItem::directLineForType(const QPointF& point1, const QPointF& point2)
{
