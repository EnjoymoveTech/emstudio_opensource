...
void ObjectItem::setExist(bool exist)
{
    m_isExist = exist;
}

bool ObjectItem::exist()
{
    return m_isExist;
}
...
void ObjectItem::updateNameItem(const Style *style)
{
    if (!suppressTextDisplay()) {
        ...
        if (style->headerFont() != m_nameItem->font())
            m_nameItem->setFont(style->normalFont());
...
void ObjectItem::updateOidItem(const Style *style)
{
    if (!suppressTextDisplay()) {
        if (!m_oidItem) {
            m_oidItem = new QGraphicsTextItem(this);
            QTextOption textOption = m_oidItem->document()->defaultTextOption();
            textOption.setAlignment(Qt::AlignHCenter);
            m_oidItem->document()->setDefaultTextOption(textOption);
            QObject::connect(m_oidItem->document(), &QTextDocument::contentsChanged, m_nameItem,
                             [=]()
            {
                this->m_oidItem->setTextWidth(-1);
                this->m_oidItem->setTextWidth(m_oidItem->boundingRect().width());
                //this->setFromDisplayName(m_oidItem->toPlainText());
            });
        }
        if (style->headerFont() != m_oidItem->font())
        {
            QFont font = style->normalFont();
            font.setPixelSize(font.pixelSize()*0.9);
            m_oidItem->setFont(font);
        }
        if (style->textBrush().color() != m_oidItem->defaultTextColor())
            m_oidItem->setDefaultTextColor(style->textBrush().color());
        if (!m_oidItem->hasFocus()) {
            m_oidItem->setPlainText("(" + m_object->oid().split(".").last() + ")");
            //m_oidItem->setEnabled(false);
        }
    } else if (m_oidItem){
        m_oidItem->scene()->removeItem(m_oidItem);
        delete m_oidItem;
        m_oidItem = nullptr;
    }
}
...
void ObjectItem::contextMenuEvent(QGraphicsSceneContextMenuEvent *event)
{
    ...
    if (addSeparator)
        menu.addSeparator();
    //change by lingfei.yu to close remove
    /*menu.addAction(new ContextMenuAction(tr("Remove"), "remove",
                                         QKeySequence(QKeySequence::Delete), &menu));*/
    //change QKeySequence by lingfei.yu
    menu.addAction(
        new ContextMenuAction(tr("Delete"), "delete", QKeySequence(QKeySequence::Delete), &menu));
