...
static const qreal MINIMUM_INNER_WIDTH = 250.0;
static const qreal MINIMUM_INNER_HEIGHT = 250.0;
static const qreal CONTENTS_BORDER_VERTICAL = 4.0;
static const qreal CONTENTS_BORDER_HORIZONTAL = 4.0;

class DeviceItem::BoundaryTextItem : public QGraphicsTextItem
{
public:
...
DeviceItem::DeviceItem(DDevice *boundary, DiagramSceneModel *diagramSceneModel, QGraphicsItem *parent)
...
void DeviceItem::update()
{
    ...
    QPen borderPen(QBrush(QColor(218, 112,214)), 1, Qt::DashLine);
    borderPen.setDashPattern(QVector<qreal>() << 10 << 10);
    m_borderItem->setPen(borderPen);

    updateSelectionMarker();
    updateGeometry();

    if(isSelected())
        setZValue(DEVICE_ITEMS_ZVALUE);
//    else
//        setZValue(-1 * DEVICE_ITEMS_ZVALUE);

    m_isUpdating = false;
}

QPointF DeviceItem::pos() const
{
    return m_boundary->pos();
}

QRectF DeviceItem::rect() const
{
    return m_boundary->rect();
}

QSizeF DeviceItem::minimumSize() const
{
    return calcMinimumGeometry();
}

void DeviceItem::setPosAndRect(const QPointF &originalPos, const QRectF &originalRect, const QPointF &topLeftDelta,
...
void DeviceItem::alignItemSizeToRaster(IResizable::Side adjustHorizontalSide, IResizable::Side adjustVerticalSide,
...
void DeviceItem::moveDelta(const QPointF &delta)
{
    m_diagramSceneModel->diagramController()->startUpdateElement(m_boundary, m_diagramSceneModel->diagram(), DiagramController::UpdateGeometry);
...
void DeviceItem::alignItemPositionToRaster(double rasterWidth, double rasterHeight)
{
    QPointF pos = m_boundary->pos();
    QRectF rect = m_boundary->rect();
...
bool DeviceItem::isSecondarySelected() const
{
    return m_isSecondarySelected;
}

void DeviceItem::setSecondarySelected(bool secondarySelected)
{
...
bool DeviceItem::isFocusSelected() const
{
    return m_isFocusSelected;
}

void DeviceItem::setFocusSelected(bool focusSelected)
{
...
QRectF DeviceItem::getSecondarySelectionBoundary()
{
    return QRectF();
}

void DeviceItem::setBoundarySelected(const QRectF &boundary, bool secondary)
{
...
bool DeviceItem::isEditable() const
{
    return true;
}

bool DeviceItem::isEditing() const
{
    return m_textItem && m_textItem->hasFocus();
}

void DeviceItem::edit()
{
    if (m_textItem)
        m_textItem->setFocus();
}

void DeviceItem::finishEdit()
{
    if (m_textItem)
...
void DeviceItem::contextMenuEvent(QGraphicsSceneContextMenuEvent *event)
{
    QMenu menu;
    menu.addAction(
        new ContextMenuAction("Delete", "delete", QKeySequence(QKeySequence::Delete), &menu));
...
void DeviceItem::updateSelectionMarker()
{
...
void DeviceItem::updateSelectionMarkerGeometry(const QRectF &boundaryRect)
{
    if (m_selectionMarker)
        m_selectionMarker->setRect(boundaryRect);
}

const Style *DeviceItem::adaptedStyle()
{
    return m_diagramSceneModel->styleController()->adaptDeviceStyle(m_boundary);
}

bool DeviceItem::sceneEventFilter(QGraphicsItem *watched, QEvent *event)
{
...
void DeviceItem::onContentsChanged()
{
...
QSizeF DeviceItem::calcMinimumGeometry() const
{
...
void DeviceItem::updateGeometry()
{
    prepareGeometryChange();
