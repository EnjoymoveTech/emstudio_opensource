...
void DFlatAssignmentVisitor::visitDDevice(const DDevice *device)
{
    visitDElement(device);
    auto target = dynamic_cast<DDevice *>(m_target);
    QMT_ASSERT(target, return);
    target->setText(device->text());
    target->setPos(device->pos());
    target->setRect(device->rect());
}
