...
class pinItem : public QTableWidgetItem
{
public:
    pinItem(QString);
signals:
...
pinItem::pinItem(QString text):QTableWidgetItem(text){};

TabDelegate::TabDelegate(const QVector<int>& w, QWidget* parent) : QStyledItemDelegate(parent)
{
    m_width = w;
}

void TabDelegate::paint(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const
{
    int sum = 0;
    for(auto i : m_width)
        sum += i;
    ...
    if (pTable != nullptr) {
        ...
        for(int i = 0; i < m_width.size(); ++i)
        {
            if (index.column() == i) {  //列标为0
                int width = int(pTable->width() * m_width[i] * 1.0 / sum)-6;    //按比例设置列宽
...
static int translateAssociationKindToIndex(MAssociationEnd::Kind kind)
{
    ...
    case MAssociationEnd::Composition:
        return 2;
    case MAssociationEnd::Method:
        return 3;
    case MAssociationEnd::Event:
        return 4;
    case MAssociationEnd::Field:
        return 5;
    case MAssociationEnd::Pin:
        return 6;
    }
    return 0;
}
...
static MAssociationEnd::Kind translateIndexToAssociationKind(int index)
{
    static const MAssociationEnd::Kind map[] = {
        ...
        MAssociationEnd::Method, MAssociationEnd::Event, MAssociationEnd::Field, MAssociationEnd::Pin
    };
...
int PropertiesView::MView::getDefaultProt()
{
    QSet<int> set;
    foreach (const MDiagram* diagram,m_propertiesView->m_diagramController->allDiagrams())
    {
        QList<DElement *> elements = diagram->diagramElements();
        foreach (DElement* element,elements)
        {
            if(dynamic_cast<DComponent*>(element))
            {
                QHashIterator<QString, QString> i(dynamic_cast<DComponent*>(element)->serviceInfo());
                while (i.hasNext()) {
                    i.next();
                    set << i.value().split(";")[1].toInt();
                }
                foreach (const QString& key,dynamic_cast<DComponent*>(element)->pinInfo().keys())
                {
                    int port = dynamic_cast<DComponent*>(element)->pinInfo().value(key).toInt();
            ...
            else if(dynamic_cast<DBoundary*>(element))
            {
                QHashIterator<QString, QString> i(dynamic_cast<DBoundary*>(element)->baseSrvInfo());
                while (i.hasNext()) {
                    i.next();
                    set << i.value().toInt();
                }
                i = dynamic_cast<DBoundary*>(element)->staticInfo();
                while (i.hasNext()) {
                    i.next();
                    set << i.value().split(";").at(0).toInt();
                }
            }
        }
    }
    return Utils::makeUniquelyId(set,1,COMPONENT_START_PORT);
}
...
void PropertiesView::MView::visitMElement(const MElement *element)
{
    ...
    //haojie.fang add oid
    if(dynamic_cast<MComponent*>(const_cast<MElement *>(element)))
    {
        if (!m_oidNameLabel) {
            m_oidNameLabel = new QLabel();
            addRow(tr("oid:"), m_oidNameLabel, "oid");
        }
        m_oidNameLabel->setText(element->oid());
    }


    //haojie.fang 关闭stereotype选择
//    if (!m_stereotypeComboBox) {
//        m_stereotypeComboBox = new QComboBox(m_referanceWidget);
...
//    }
    ...
    if (!m_reverseEngineeredLabel) {
        m_reverseEngineeredLabel = new QLabel(m_referanceWidget);
...
void PropertiesView::MView::visitMObject(const MObject *object)
{
    ...
    if (!m_elementNameLineEdit) {
        m_elementNameLineEdit = new QLineEdit(m_referanceWidget);
...
void PropertiesView::MView::visitMClass(const MClass *klass)
{
    ...
    if (!m_namespaceLineEdit) {
        m_namespaceLineEdit = new QLineEdit(m_referanceWidget);
    ...
    if (!m_templateParametersLineEdit) {
        m_templateParametersLineEdit = new QLineEdit(m_referanceWidget);
    ...
    if (!m_classMembersStatusLabel) {
        ...
        m_classMembersStatusLabel = new QLabel(m_referanceWidget);
        m_classMembersParseButton = new QPushButton(tr("Clean Up"), m_referanceWidget);
    ...
    if (!m_classMembersEdit) {
        m_classMembersEdit = new ClassMembersEdit(m_referanceWidget);
...
void PropertiesView::MView::visitMComponent(const MComponent *component)
{
    ...
    EmosTools::IDLStruct idlStruct = EmosTools::IdlParser::getIdlStruct(component->oid());
    if(idlStruct.isValid)
    {
        showServiceInfo(component, idlStruct);
        showPropertyInfo(component, idlStruct);

        //添加详细信息界面内容，使用的是idl的inspector界面：
        if(!m_detailInfoView)
        {
            m_detailInfoView = new Idl::Internal::InspectorTreeView();
            m_detailInfoView->setModel((QAbstractItemModel*)&m_inspectorModel);
            m_detailInfoLayout = new QVBoxLayout(m_detailInfoView);
            m_detailInfoView->setLayout(m_detailInfoLayout);
            m_topLayout->addWidget(m_detailInfoView);
        }
        QDomDocument doc = EmosTools::IdlParser::getIdlDocument(component->oid());
        m_inspectorModel.LoadFromDoc(&doc);
        m_detailInfoView->expandAll();
    }
}
...
#ifdef SHOW_DEBUG_PROPERTIES
    if (!m_diagramsLabel) {
        m_diagramsLabel = new QLabel(m_referanceWidget);
...
void PropertiesView::MView::visitMItem(const MItem *item)
{
    ...
    if (item->isVarietyEditable()) {
        if (!m_itemVarietyEdit) {
            m_itemVarietyEdit = new QLineEdit(m_referanceWidget);
...
void PropertiesView::MView::visitMDependency(const MDependency *dependency)
{
    ...
    if (!m_directionSelector) {
        m_directionSelector = new QComboBox(m_referanceWidget);
...
{//haojie.fang Association属性窗口

    //haojie.fang 属性窗口名使用 kind
    ...
    bool isSingleSelection = selection.size() == 1;
    if (!m_endALabel) {
        m_endALabel = new QLabel("<b>" + m_endAName + " (" + association->endAOid() + ")</b>");
        addRow(m_endALabel, "end a");
    }

    /*
    if (!m_endAEndName) {
        m_endAEndName = new QLineEdit(m_referanceWidget);
    ...
    if (!m_endACardinality) {
        m_endACardinality = new QLineEdit(m_referanceWidget);
    ...
    if (!m_endANavigable) {
        m_endANavigable = new QCheckBox(tr("Navigable"), m_referanceWidget);
    ...
    if (!m_endAKind) {
        m_endAKind = new QComboBox(m_referanceWidget);
        m_endAKind->addItems({ tr("Method"), tr("Event"), tr("Field"), tr("Pin") });//haojie.fang 只保留三种选择
        //m_endAKind->addItems({ tr("Association"), tr("Aggregation"), tr("Composition"),tr("Method"), tr("Event"), tr("Field"), tr("Pin) });
        m_endAKind->setCurrentIndex(association->endA().kind() - MAssociationEnd::Method);
        m_endAKind->setEnabled(false);
...
//                this, &PropertiesView::MView::onAssociationEndAKindChanged);
    ...
    */
    ...
    if (!m_endBLabel) {
        m_endBLabel = new QLabel("<b>" + m_endBName + " (" + association->endBOid() + ")</b>");
        addRow(m_endBLabel, "end b");
    }

    /*
    if (!m_endBEndName) {
        m_endBEndName = new QLineEdit(m_referanceWidget);
    ...
    if (!m_endBCardinality) {
        m_endBCardinality = new QLineEdit(m_referanceWidget);
    ...
    if (!m_endBNavigable) {
        m_endBNavigable = new QCheckBox(tr("Navigable"), m_referanceWidget);
    ...
    if (m_endBNavigable->isEnabled() != isSingleSelection)
        ...
        */
    if (!m_endBKind) {
        m_endBKind = new QComboBox(m_referanceWidget);
        m_endBKind->addItems({ tr("Method"), tr("Event"), tr("Field"), tr("Pin") });//haojie.fang 只保留三种选择
        //m_endBKind->addItems({ tr("Association"), tr("Aggregation"), tr("Composition"),tr("Method"), tr("Event"), tr("Field"), tr("Pin") });
        m_endBKind->setCurrentIndex(association->endB().kind() - MAssociationEnd::Method);
        m_endBKind->setEnabled(false);
...
//                this, &PropertiesView::MView::onAssociationEndBKindChanged);
    ...
    //haojie.fang runnable 和 event / field 对应选择
    eventAssociationWidget(association);
    fieldAssociationWidget(association);
    pinAssociationWidget(association);
}
...
#ifdef SHOW_DEBUG_PROPERTIES
        if (!m_separatorLine) {
            m_separatorLine = new QFrame(m_referanceWidget);
...
void PropertiesView::MView::visitDObject(const DObject *object)
{
...
#ifdef SHOW_DEBUG_PROPERTIES
    if (!m_posRectLabel) {
        m_posRectLabel = new QLabel(m_referanceWidget);
    ...
    if (!m_autoSizedCheckbox) {
        m_autoSizedCheckbox = new QCheckBox(tr("Auto sized"), m_referanceWidget);
    ...
    if (!m_visualPrimaryRoleSelector) {
        m_visualPrimaryRoleSelector = new PaletteBox(m_referanceWidget);
    ...
    if (!m_visualSecondaryRoleSelector) {
        m_visualSecondaryRoleSelector = new QComboBox(m_referanceWidget);
    ...
    if (!m_visualEmphasizedCheckbox) {
        m_visualEmphasizedCheckbox = new QCheckBox(tr("Emphasized"), m_referanceWidget);
...
//        m_stereotypeDisplaySelector = new QComboBox(m_referanceWidget);
...
#ifdef SHOW_DEBUG_PROPERTIES
    if (!m_depthLabel) {
        m_depthLabel = new QLabel(m_referanceWidget);
...
void PropertiesView::MView::visitDClass(const DClass *klass)
{
    ...
    if (!m_templateDisplaySelector) {
        m_templateDisplaySelector = new QComboBox(m_referanceWidget);
    ...
    if (!m_showAllMembersCheckbox) {
        m_showAllMembersCheckbox = new QCheckBox(tr("Show members"), m_referanceWidget);
...
//        m_plainShapeCheckbox = new QCheckBox(tr("Plain shape"), m_referanceWidget);
...
void PropertiesView::MView::visitDItem(const DItem *item)
{
    ...
    if (item->isShapeEditable()) {
        if (!m_itemShapeEdit) {
            m_itemShapeEdit = new QLineEdit(m_referanceWidget);
...
#ifdef SHOW_DEBUG_PROPERTIES
    if (!m_pointsLabel) {
        m_pointsLabel = new QLabel(m_referanceWidget);
...
void PropertiesView::MView::visitDAnnotation(const DAnnotation *annotation)
{
    ...
    if (!m_annotationAutoWidthCheckbox) {
        m_annotationAutoWidthCheckbox = new QCheckBox(tr("Auto width"), m_referanceWidget);
    ...
    if (!m_annotationVisualRoleSelector) {
        m_annotationVisualRoleSelector = new QComboBox(m_referanceWidget);
...
{
    setTitle<DBoundary>(m_diagramElements, tr("Process"), tr("Processes"));
    ...
    if (!m_elementNameLineEdit) {
        ...
        addRow(tr("Name:"), m_elementNameLineEdit, "name");
        connect(m_elementNameLineEdit, &QLineEdit::textChanged,
                this, &PropertiesView::MView::onBoundryNameChanged);
    }
    if (boundary->text() != m_elementNameLineEdit->text() && !m_elementNameLineEdit->hasFocus())
        m_elementNameLineEdit->setText(boundary->text());

    if(!m_staticWidget)
    {
        m_staticWidget = new ProcessStaticWidget(boundary, this);
        addRow("Static", m_staticWidget, "Static");
        connect(m_staticWidget, &ProcessStaticWidget::dataChanged,
                this, &PropertiesView::MView::onStaticChanged);
        connect(m_staticWidget, &ProcessStaticWidget::selectChanged,
                this, &PropertiesView::MView::onStaticSelectChanged);
        m_staticWidget->load(boundary, this);
    }

    if(!m_baseSrvWidget)
    {
        m_baseSrvWidget = new BaseSrvWidget(m_referanceWidget);
        addRow("Service", m_baseSrvWidget, "Service");
        connect(m_baseSrvWidget, &BaseSrvWidget::dataChanged,
                this, &PropertiesView::MView::onBaseSrvWidgetChanged);
        connect(m_baseSrvWidget, &BaseSrvWidget::selectChanged,
                this, &PropertiesView::MView::onBaseSrvWidgetSelectChanged);
        m_baseSrvWidget->load(boundary);
    }
}

void PropertiesView::MView::visitDDevice(const DDevice *boundary)
{
    setTitle<DDevice>(m_diagramElements, tr("Device"), tr("Device"));
    visitDElement(boundary);
    ...
    if (!m_elementNameLineEdit) {
        ...
        addRow(tr("Name:"), m_elementNameLineEdit, "name");
        connect(m_elementNameLineEdit, &QLineEdit::textChanged,
                this, &PropertiesView::MView::onDeviceNameChanged);
    }
    if (boundary->text() != m_elementNameLineEdit->text() && !m_elementNameLineEdit->hasFocus())
        m_elementNameLineEdit->setText(boundary->text());
    if (!m_serviceAddress) {
        m_serviceAddress = new QLineEdit(m_referanceWidget);
        m_serviceAddress->setText(boundary->ip());
        addRow(tr("Address:"), m_serviceAddress, "address");
        connect(m_serviceAddress, &QLineEdit::textEdited, this,
                &PropertiesView::MView::onDeviceAddressChanged);
    }
    if (!m_ecuID) {
        m_ecuID = new QLineEdit(m_referanceWidget);
        m_ecuID->setText(QString::number(boundary->ecuID()));
        addRow(tr("ecuID:"), m_ecuID, "ecuID");
        connect(m_ecuID, &QLineEdit::textEdited, this,
                &PropertiesView::MView::onDeviceEcuIDChanged);
    }
}
...
void PropertiesView::MView::onBoundryNameChanged(const QString &name)
{
    assignModelElement<DBoundary, QString>(m_diagramElements, SelectionSingle, name, &DBoundary::text, &DBoundary::setText);
}

void PropertiesView::MView::onDeviceNameChanged(const QString &name)
{
    assignModelElement<DDevice, QString>(m_diagramElements, SelectionSingle, name, &DDevice::text, &DDevice::setText);
}
...
QHash<QString, QString> PropertiesView::MView::onGetCurrentServiceInfo(const QString & text)
{
    m_serviceHash.insert(text, setServiceInfoToString("127.0.0.1",
                                                             m_servicePort->text().toInt(),
                                                             index2Protocol().value(m_serviceProtocol->currentIndex()),
                                                             index2Type().value(m_serviceType->currentIndex())));

    qDebug() << text << m_serviceHash.value(text);

    return m_serviceHash;
}

void PropertiesView::MView::onServiceAddressChanged()
{
    assignModelElement<MComponent, QHash <QString,QString>>(
                m_modelElements, SelectionSingle, onGetCurrentServiceInfo(m_serviceListWidget->currentItem()->text()), &MComponent::serviceInfo, &MComponent::setServiceInfo);

    assignModelElement<DComponent, QHash <QString,QString>>(
                m_diagramElements, SelectionSingle, onGetCurrentServiceInfo(m_serviceListWidget->currentItem()->text()), &DComponent::serviceInfo, &DComponent::setServiceInfo);
}

void PropertiesView::MView::showServiceInfo(const MComponent *component, const EmosTools::IDLStruct &idl)
{
    if(idl.service.isEmpty())
        return;

    if(!m_serviceListWidget)
    {
        m_serviceHash = component->serviceInfo();

        QList<QString> delServiceHash;
        QHashIterator<QString, QString> i(m_serviceHash);
        while (i.hasNext()) {
            i.next();
            delServiceHash.push_back(i.key());
        }

        m_serviceListWidget = new QListWidget(m_referanceWidget);
        m_serviceListWidget->setFlow(QListView::LeftToRight);
        m_serviceListWidget->setFixedHeight(25);
        addRow(tr("ServiceList:"), m_serviceListWidget, "serviceList");

        bool isAddSave = false;

        QMapIterator<QString, int> serviceNode(idl.service);
        while (serviceNode.hasNext()) {
            serviceNode.next();

            QString sName = serviceNode.key();
            EmosTools::tSrvService srv = EmosTools::ApiParser::getInstance()->getService(sName);
            if (!srv.isValid())
            {
                continue;
            }
            if(!m_serviceHash.contains(sName))
            {
                isAddSave = true;
                m_serviceHash.insert(sName, setServiceInfoToString("127.0.0.1",getDefaultProt(),1,2));
            }
            delServiceHash.removeOne(sName);
            m_serviceListWidget->addItem(sName);
        }

        if(m_serviceListWidget->count() > 0)
            m_serviceListWidget->setCurrentRow(0);

        connect(m_serviceListWidget, &QListWidget::currentTextChanged, [&](const QString& text){
            qDebug()<<m_serviceHash;

            disconnect(m_servicePort, &QLineEdit::textEdited, this, &PropertiesView::MView::onServiceAddressChanged);
            disconnect(m_serviceProtocol, &QComboBox::currentTextChanged, this, &PropertiesView::MView::onServiceAddressChanged);
            disconnect(m_serviceType, &QComboBox::currentTextChanged, this, &PropertiesView::MView::onServiceAddressChanged);

            //address目前保持一致
            //m_serviceAddress->setText(getAddrFromString(m_serviceHash, text));
            m_servicePort->setText(QString::number(getPortFromString(m_serviceHash, text)));
            m_serviceProtocol->setCurrentIndex(protocol2Index().value(getProtocolFromString(m_serviceHash, text)));
            m_serviceType->setCurrentIndex(type2Index().value(getTypeFromString(m_serviceHash, text)));

            connect(m_servicePort, &QLineEdit::textEdited, this, &PropertiesView::MView::onServiceAddressChanged);
            connect(m_serviceProtocol, &QComboBox::currentTextChanged, this, &PropertiesView::MView::onServiceAddressChanged);
            connect(m_serviceType, &QComboBox::currentTextChanged, this, &PropertiesView::MView::onServiceAddressChanged);

            if(!m_serviceHash.contains(text))
                onServiceAddressChanged();
        });
        QString serviceName;
        if (m_serviceListWidget->count() > 0)
        {
        serviceName = m_serviceListWidget->currentItem()->text();

//                if (!m_serviceAddress) {
//                    m_serviceAddress = new QLineEdit(m_referanceWidget);
//                    QString address = m_serviceHash.contains(serviceName) ? m_serviceHash.value(serviceName).split(";")[0] : "127.0.0.1";
//                    m_serviceAddress->setText(address);
//                    addRow(tr("ServiceAddress:"), m_serviceAddress, "serviceAddress");
//                    connect(m_serviceAddress, &QLineEdit::textEdited,[&](){
//                        onServiceAddressChanged(m_serviceListWidget->currentItem()->text());
//                    });
//                }
        if (!m_servicePort) {
            m_servicePort = new QLineEdit(m_referanceWidget);
            m_servicePort->setText(QString::number(getPortFromString(m_serviceHash, serviceName)));
            addRow(tr("ServicePort:"), m_servicePort, "servicePort");
            connect(m_servicePort, &QLineEdit::textEdited, this, &PropertiesView::MView::onServiceAddressChanged);
        }
        if (!m_serviceProtocol) {
            m_serviceProtocol = new QComboBox(m_referanceWidget);
            m_serviceProtocol->addItems(QStringList() << "TRANSTYPE_TCP" << "TRANSTYPE_IPC");
            m_serviceProtocol->setCurrentIndex(protocol2Index().value(getProtocolFromString(m_serviceHash, serviceName)));
            addRow(tr("ServiceProtocol:"), m_serviceProtocol, "serviceProtocol");
            connect(m_serviceProtocol, &QComboBox::currentTextChanged, this, &PropertiesView::MView::onServiceAddressChanged);
        }
        if (!m_serviceType) {
            m_serviceType = new QComboBox(m_referanceWidget);
            m_serviceType->addItems(QStringList() << "SD_AF_INET" << "SD_AF_IPC");
            m_serviceType->setCurrentIndex(type2Index().value(getTypeFromString(m_serviceHash, serviceName)));
            addRow(tr("ServiceType:"), m_serviceType, "serviceType");
            connect(m_serviceType, &QComboBox::currentTextChanged, this, &PropertiesView::MView::onServiceAddressChanged);
        }

        for(const auto& del : delServiceHash)
        {
            m_serviceHash.remove(del);
        }
        if(delServiceHash.size() > 0 || isAddSave)
            onServiceAddressChanged();
        }
    }
}

void PropertiesView::MView::showPropertyInfo(const MComponent *component, const EmosTools::IDLStruct &idl)
{
    if(idl.property.isEmpty())
       return;

    if(!m_propertyTableView)
    {
        QHash<QString, QString> propertyInfo = component->propertyInfo();

        m_propertyTableView = new QTableView(m_referanceWidget);
        m_propertyTableModel = new QStandardItemModel(m_propertyTableView);
        m_propertyTableView->setModel(m_propertyTableModel);
        m_propertyTableView->setItemDelegate(new TabDelegate(QVector<int>()<< 1 << 1 << 1, m_propertyTableView));
        m_propertyTableModel->setHorizontalHeaderLabels(QStringList()<<"Name"<<"Type"<<"Value");
        addRow(tr("Property:"), m_propertyTableView, "property");

        QMapIterator<QString, QString> propertyNode(idl.property);
        int row = 0;
        while (propertyNode.hasNext()) {
            propertyNode.next();

            QString name = propertyNode.key();
            QString type = propertyNode.value();
            QString value = "";
            if(propertyInfo.contains(name))
            {
                value = propertyInfo.value(name).split(";").last();
            }
            m_propertyTableModel->appendRow(QList<QStandardItem*>()
            ...
            if(type == "Bool")
            {
                ...
                m_propertyTableView->setIndexWidget(m_propertyTableModel->index(row, 2), box);
                if(value == "")
                {
                    value = "true";
                    box->setCurrentText(value);
                    onPropertyTableChanged();
                }
                box->setCurrentText(value);
                connect(box, &QComboBox::currentTextChanged,
                        this, &PropertiesView::MView::onPropertyTableChanged);
            }
            row++;
        }

        for(int i = 0; i < m_propertyTableModel->rowCount(); i++)
        {
            m_propertyTableModel->item(i, 0)->setFlags(m_propertyTableModel->item(i,0)->flags() & ~Qt::ItemIsEditable);
            m_propertyTableModel->item(i, 1)->setFlags(m_propertyTableModel->item(i,1)->flags() & ~Qt::ItemIsEditable);
        }
        connect(m_propertyTableModel, &QStandardItemModel::itemChanged,
                this, &PropertiesView::MView::onPropertyTableChanged);
    }
}

void PropertiesView::MView::onStaticChanged(const QHash<QString, QString> &staticInfo)
{
    assignModelElement<DBoundary, QHash<QString, QString>>(
                m_diagramElements, SelectionSingle, staticInfo, &DBoundary::staticInfo, &DBoundary::setStaticInfo);
}

void PropertiesView::MView::onStaticSelectChanged(bool checked)
{
    assignModelElement<DBoundary, bool>(
                m_diagramElements, SelectionSingle, checked, &DBoundary::staticSelected, &DBoundary::setStaticSelected);
}

void PropertiesView::MView::onPropertyTableChanged()
{
    QHash <QString,QString> property;
    for(int i = 0; i < m_propertyTableModel->rowCount(); i++)
    {
        QString type = m_propertyTableModel->item(i, 1)->text();
        if(type == "Bool")
        {
            QModelIndex index = m_propertyTableModel->index(i, 2, QModelIndex());
            QComboBox* box =  qobject_cast<QComboBox*>(m_propertyTableView->indexWidget(index));
            if(box)
            {
                property.insert(m_propertyTableModel->item(i, 0)->text(), m_propertyTableModel->item(i, 1)->text() + ";" + box->currentText());
            }
        }
        else
        {
            property.insert(m_propertyTableModel->item(i, 0)->text(), m_propertyTableModel->item(i, 1)->text() + ";" + m_propertyTableModel->item(i, 2)->text());
        }
    }

    assignModelElement<MComponent, QHash <QString,QString>>(
                m_modelElements, SelectionSingle, property, &MComponent::propertyInfo, &MComponent::setPropertyInfo);

    assignModelElement<DComponent, QHash <QString,QString>>(
                m_diagramElements, SelectionSingle, property, &DComponent::propertyInfo, &DComponent::setPropertyInfo);
}

void PropertiesView::MView::onBaseSrvWidgetChanged(const QHash<QString, QString> &propertyInfo)
{
    assignModelElement<DBoundary, QHash <QString,QString>>(
                m_diagramElements, SelectionSingle, propertyInfo, &DBoundary::baseSrvInfo, &DBoundary::setBaseSrvInfo);
}

void PropertiesView::MView::onBaseSrvWidgetSelectChanged(const QList<QString> &selectSrv)
{
    assignModelElement<DBoundary, QList <QString>>(
                m_diagramElements, SelectionSingle, selectSrv, &DBoundary::baseSrvSelect, &DBoundary::setBaseSrvSelect);
}

void PropertiesView::MView::onDeviceAddressChanged(const QString &addr)
{
    assignModelElement<DDevice, QString>(
                m_diagramElements, SelectionSingle, addr, &DDevice::ip, &DDevice::setIp);
}

void PropertiesView::MView::onDeviceEcuIDChanged(const QString &ecuid)
{
    assignModelElement<DDevice, int>(
                m_diagramElements, SelectionSingle, ecuid.toInt(), &DDevice::ecuID, &DDevice::setEcuID);
}

void PropertiesView::MView::onDeviceTableChanged(QStandardItem *item)
{
    QString ip;
    if(item->checkState() == Qt::Checked && item->column() == 0)
    {
        for(int i = 0; i < m_deviceTableModel->rowCount(); i++)
        {
            if(m_deviceTableModel->item(i, 0) != item)
                m_deviceTableModel->item(i, 0)->setCheckState(Qt::Unchecked);
            else
            {
                ip = m_deviceTableModel->item(i, 1)->text();
            }
        }
        assignModelElement<DDevice, QString>(
                    m_diagramElements, SelectionSingle, ip, &DDevice::ip, &DDevice::setIp);
    }
}

//haojie.fang add event Map
void PropertiesView::MView::onAssociationEventChanged(const QString&)
{
    if(m_detailEventTable)
    {
        QHash<QString,QString> eventMap;

        for(int i = 0; i< m_detailEventTable->rowCount(); i++)
        {
            QComboBox* box = reinterpret_cast<QComboBox*>(m_detailEventTable->cellWidget(i,1));

            if(box->currentText() == "")
                continue;

            eventMap.insert(m_detailEventTable->item(i,0)->text(), box->currentText());
        }

        assignModelElement<MAssociation, QHash<QString,QString>>(
                    m_modelElements, SelectionSingle, eventMap, &MAssociation::getEventMap, &MAssociation::setEventMap);


        assignModelElement<DAssociation, QHash<QString,QString>>(
                    m_diagramElements, SelectionSingle, eventMap, &DAssociation::getEventMap, &DAssociation::setEventMap);
    }

}

void PropertiesView::MView::onAssociationFieldChanged(const QString&)
{
    if(m_detailEventTable)
    {
        ...
        for(int i = 0; i< m_detailEventTable->rowCount(); i++)
        {
            ...
            fieldMap.insert(m_detailEventTable->item(i,0)->text(), box->currentText());
        }

        assignModelElement<MAssociation, QHash<QString,QString>>(
                    m_modelElements, SelectionSingle, fieldMap, &MAssociation::getFieldMap, &MAssociation::setFieldMap);

        assignModelElement<DAssociation, QHash<QString,QString>>(
                    m_diagramElements, SelectionSingle, fieldMap, &DAssociation::getFieldMap, &DAssociation::setFieldMap);
    }

}

void PropertiesView::MView::onAssociationPinChanged(const QString &)
{
    if(m_detailEventTable)
    {
        QHash<QString,QString> pinMap;
        ...
        for(int i = 0; i< m_detailEventTable->rowCount(); i++)
        {
            ...
            if(box->currentText() == "")
                continue;
            //EmosTools::tSrvService srv = api->getService(box->currentText().split(":").at(0).trimmed());
            //
            pinMap.insert(m_detailEventTable->item(i,0)->text(), box->currentText());
        }

        assignModelElement<MAssociation, QHash<QString,QString>>(
                    m_modelElements, SelectionSingle, pinMap, &MAssociation::getPinMap, &MAssociation::setPinMap);

        assignModelElement<DAssociation, QHash<QString,QString>>(
                    m_diagramElements, SelectionSingle, pinMap, &DAssociation::getPinMap, &DAssociation::setPinMap);
    }
}

void PropertiesView::MView::onAssociationPinPortChanged(QTableWidgetItem *item)
{
    if (item->column() == 2)
    {
        if(m_detailEventTable)
        {
            QComboBox* box = reinterpret_cast<QComboBox*>(m_detailEventTable->cellWidget(item->row(),1));
            if(box->currentText() == "")
                return;
            if (item->text().isEmpty())
            {
                item->setText(QString::number(getDefaultProt()));
                return;
            }
            //QStringList  strlist = box->currentText().split(":",QString::SkipEmptyParts);
            QPair<QString,QString> pair(box->currentText(),item->text());
           // assignModelElement<MComponent, QPair<QString,QString>>(
                        //m_modelElements, SelectionSingle, pair, &MAssociation::getPinMap, &MAssociation::setPinMap);
            MElement* element = m_propertiesView->m_modelController->findElement(m_uid);


            assignModelElement<MComponent, QPair<QString,QString>>(
                        QList<qmt::MElement* >()<<element, SelectionSingle, pair,box->currentText(), &MComponent::findPinInfo, &MComponent::addPinInfo);
            assignModelElement<DComponent, QPair<QString,QString>>(
                        QList<qmt::DElement* >()<<m_propertiesView->m_diagramController->findDelegate(element,m_diagram), SelectionSingle, pair,box->currentText(), &DComponent::findPinInfo, &DComponent::addPinInfo);
        }
    }
}

void PropertiesView::MView::eventAssociationWidget(const MAssociation *association)
{
    if(association->endA().kind() == MAssociationEnd::Event &&
       association->endB().kind() == MAssociationEnd::Event)
    {
        EmosTools::IDLStruct structA = EmosTools::IdlParser::getIdlStruct(association->endAOid());
        EmosTools::IDLStruct structB = EmosTools::IdlParser::getIdlStruct(association->endBOid());
        //QDomDocument docB = EmosTools::IdlParser::getIdlDocument(association->endBOid());
        if(structB.isValid)
        {
            //QDomNodeList threadList = docB.elementsByTagName("Thread");
            QList<EmosTools::ThreadDataStruct> threadlist = structB.thread;
            //QList<QDomNode> eventRunable;
            QList<EmosTools::ThreadDataStruct> eventlist;
            for(int i = 0; i < threadlist.size(); ++i)
            {
                if(threadlist.at(i).type == "Event")
                {
                    eventlist.push_back(threadlist.at(i));
                }
            }

            if(!m_detailEventTable)
                m_detailEventTable = new QTableWidget();
            m_detailEventTable->clear();
            m_detailEventTable->setColumnCount(2);
            m_detailEventTable->setHorizontalHeaderLabels(QStringList()<<"Runable"<<"Event");
            m_detailEventTable->setRowCount(eventlist.size());
            m_detailEventTable->horizontalHeader()->setSectionResizeMode(QHeaderView::Stretch);

            bool isNoData = false;
            QHash<QString,QString> eventMap = association->getEventMap();
            if(eventMap.size() == 0)
                isNoData = true;

            for(int i = 0; i < eventlist.size(); ++i)
            {
                EmosTools::ThreadDataStruct event = eventlist.at(i);

                QTableWidgetItem* checkItem = new QTableWidgetItem(event.name);
                //checkItem->setCheckState(Qt::Checked);

                QString data = event.data;
                m_detailEventTable->setItem(i,0,checkItem);

                QComboBox* box = new QComboBox(m_detailEventTable);
                box->addItem("");//增加空的选择项
                QMap<QString, int> quotesrvlist = structB.quoteService;
                //QDomNodeList quoteServiceList = docB.elementsByTagName("QuoteService");
                foreach (const QString& srvName,quotesrvlist.keys())
                {
                    if(!structA.service.contains(srvName))//当前连线的idl没有这个服务则不显示
                        continue;
                    ...
                    QList<EmosTools::tSrvEvent> eventlist = srv.events;
                    foreach (const EmosTools::tSrvEvent& event,eventlist)
                    {
                        if(data.trimmed() == event.data.trimmed())//数据类型一直才能被选择
                            box->addItem(srvName + " : " + event.name);
                        //eventElement = eventElement.nextSiblingElement("Event");
                    }
                }

                m_detailEventTable->setCellWidget(i,1,box);

                box->setCurrentText(eventMap.value(checkItem->text()));

                if(isNoData)
                {
                    eventMap.insert(checkItem->text(),box->currentText());
                }

                connect(box, &QComboBox::currentTextChanged, this, &PropertiesView::MView::onAssociationEventChanged);

                m_detailEventTable->item(i,0)->setFlags(m_detailEventTable->item(i,0)->flags() & ~Qt::ItemIsEditable);

                box->setEnabled(true);
                if(hasRunableConnected(association, event.name))
                    box->setEnabled(false);
            }

            if(isNoData)
            {
                const_cast<MAssociation*>(association)->setEventMap(eventMap);
            }

            m_topLayout->addWidget(m_detailEventTable);
        }
    }
}

void PropertiesView::MView::fieldAssociationWidget(const MAssociation *association)
{
    if(association->endA().kind() == MAssociationEnd::Field && association->endB().kind() == MAssociationEnd::Field)
    {
        EmosTools::IDLStruct structA = EmosTools::IdlParser::getIdlStruct(association->endAOid());
        EmosTools::IDLStruct structB = EmosTools::IdlParser::getIdlStruct(association->endBOid());
        //QDomDocument docB = EmosTools::IdlParser::getIdlDocument(association->endBOid());
        if(structB.isValid)
        {
            //QDomNodeList threadList = docB.elementsByTagName("Thread");
            QList<EmosTools::ThreadDataStruct> threadlist = structB.thread;
            //QList<QDomNode> fieldRunable;
            QList<EmosTools::ThreadDataStruct> fieldlist;
            for(int i = 0; i < threadlist.size(); ++i)
            {
                if(threadlist.at(i).type == "Field")
                {
                    fieldlist.push_back(threadlist.at(i));
                }
            }
            ...
            m_detailEventTable->clear();
            m_detailEventTable->setColumnCount(2);
            m_detailEventTable->setHorizontalHeaderLabels(QStringList()<<"Runable"<<"Field");
            m_detailEventTable->setRowCount(fieldlist.size());
            m_detailEventTable->horizontalHeader()->setSectionResizeMode(QHeaderView::Stretch);

            bool isNoData = false;
            QHash<QString,QString> fieldMap = association->getFieldMap();
            if(fieldMap.size() == 0)
                isNoData = true;

            for(int i = 0; i < fieldlist.size(); ++i)
            {
                EmosTools::ThreadDataStruct field = fieldlist.at(i);

                QTableWidgetItem* checkItem = new QTableWidgetItem(field.name);
                //checkItem->setCheckState(Qt::Checked);

                QString data = field.data;
                m_detailEventTable->setItem(i,0,checkItem);
                ...
                foreach (const QString& srvName,quotesrvlist.keys())
                {
                    ...
                    QList<EmosTools::tSrvField> fieldlist = srv.fields;
                    //QDomElement fieldElement = quoteServiceList.at(j).firstChildElement("Field");
                    foreach (const EmosTools::tSrvField field,fieldlist)
                    {
                        if(data.trimmed() == field.data.trimmed())//数据类型一直才能被选择
                            box->addItem(srvName + " : " + field.name);
                ...
                box->setCurrentText(fieldMap.value(checkItem->text()));

                if(isNoData)
                {
                    fieldMap.insert(checkItem->text(),box->currentText());
                }

                connect(box, &QComboBox::currentTextChanged, this, &PropertiesView::MView::onAssociationFieldChanged);

                m_detailEventTable->item(i,0)->setFlags(m_detailEventTable->item(i,0)->flags() & ~Qt::ItemIsEditable);

                box->setEnabled(true);
                if(hasRunableConnected(association, field.name))
                    box->setEnabled(false);
            }

            if(isNoData)
            {
                const_cast<MAssociation*>(association)->setFieldMap(fieldMap);
            }
...
void PropertiesView::MView::pinAssociationWidget(const MAssociation *association)
{
    if(association->endA().kind() == MAssociationEnd::Pin && association->endB().kind() == MAssociationEnd::Pin)
    {
        EmosTools::IDLStruct structA = EmosTools::IdlParser::getIdlStruct(association->endAOid());
        EmosTools::IDLStruct structB = EmosTools::IdlParser::getIdlStruct(association->endBOid());
        //QDomDocument docB = EmosTools::IdlParser::getIdlDocument(association->endBOid());
        if(structB.isValid)
        {
            ...
            QList<EmosTools::ThreadDataStruct> pinlist;
            for(int i = 0; i < threadList.size(); ++i)
            {
                if(threadList.at(i).type == "Data")
                {
                    pinlist.push_back(threadList.at(i));
                }
            }
            ...
            if(!m_detailEventTable)
                m_detailEventTable = new QTableWidget();
            m_detailEventTable->clear();
            m_detailEventTable->setColumnCount(3);
            m_detailEventTable->setHorizontalHeaderLabels(QStringList()<<"Runable"<<"Pin"<<"PinPort");
            m_detailEventTable->setRowCount(pinlist.size());
            m_detailEventTable->horizontalHeader()->setSectionResizeMode(QHeaderView::Stretch);
            connect(m_detailEventTable,&QTableWidget::itemChanged,this,&PropertiesView::MView::onAssociationPinPortChanged,Qt::UniqueConnection);
            bool isNoData = false;
            QHash<QString,QString> pinMap = association->getPinMap();
            if(pinMap.size() == 0)
                isNoData = true;
            QHash<QString,QString> pinInfoHash;
            MElement* element = m_propertiesView->m_modelController->findElement(association->endAUid());
            //DElement* dia = m_propertiesView->m_diagramController->findDelegate(element,m_diagram);
            m_uid = association->endAUid();
            //DElement* element = m_propertiesView->m_diagramController->findDelegate()
            if(element)
            {
                pinInfoHash = dynamic_cast<MComponent*>(element)->pinInfo();
            }
            foreach (const QString& key,pinInfoHash.keys())
            {
                QString srvName = key.split(":").at(0).trimmed();
                if (EmosTools::ApiParser::getInstance()->getService(srvName).id == -1)
                {
                    QPair<QString,QString> pair = {key,pinInfoHash.value(key)};
                    dynamic_cast<MComponent*>(element)->removePinInfo(pair);
                    dynamic_cast<DComponent*>(m_propertiesView->m_diagramController->findDelegate(element,m_diagram))->removePinInfo(pair);
                }
            }

            for(int i = 0; i < pinlist.size(); ++i)
            {
                EmosTools::ThreadDataStruct pin = pinlist.at(i);

                QTableWidgetItem* checkItem = new QTableWidgetItem(pin.name);
                //checkItem->setCheckState(Qt::Checked);

                QString data = pin.data;
                m_detailEventTable->setItem(i,0,checkItem);

                QComboBox* box = new QComboBox(m_detailEventTable);
                box->addItem("");
                QMap<QString, int> quoteServiceList = structB.quoteService;
                foreach (const QString& srvName,quoteServiceList.keys())
                {
                    if(!structA.service.contains(srvName))//当前连线的idl没有这个服务则不显示
                        continue;
                    EmosTools::tSrvService srv = EmosTools::ApiParser::getInstance()->getService(srvName);
                    ...
                    QList<EmosTools::tSrvPin> pinlist = srv.pins;

                    //QDomElement fieldElement = quoteServiceList.at(j).firstChildElement("Pin");
                    foreach (const EmosTools::tSrvPin& pin,pinlist)
                    {
                        if(data.trimmed() == pin.data.trimmed())//数据类型一直才能被选择
                            box->addItem(srvName + " : " + pin.name);
                    }
                }

                m_detailEventTable->setCellWidget(i,1,box);

                box->setCurrentText(pinMap.value(checkItem->text()));

                if(isNoData)
                {
                    pinMap.insert(checkItem->text(),box->currentText());
                }

                connect(box, &QComboBox::currentTextChanged, this, &PropertiesView::MView::onAssociationPinChanged);



                box->setEnabled(true);
                m_detailEventTable->item(i,0)->setFlags(m_detailEventTable->item(i,0)->flags() & ~Qt::ItemIsEditable);
                if(hasRunableConnected(association, pin.name))
                    box->setEnabled(false);
                QString outpinName = QString();
                if (box->currentText() != "")
                {
                    outpinName = box->currentText();
                }
                QString pinPort = QString();
                bool findpin = false;
                if (pinPort.isEmpty() && !outpinName.isEmpty())
                {
                   pinPort = QString::number(getDefaultProt());
                }
                foreach (const QString& pinName,pinInfoHash.keys())
                {
                    if (pinName == outpinName)
                    {
                        findpin = true;
                        pinPort = pinInfoHash.value(pinName);
                        break;
                    }
                }
                pinItem* portItem = new pinItem(pinPort);
                m_detailEventTable->setItem(i,2,portItem);
                if (!findpin)
                {
                    m_detailEventTable->item(i,2)->setFlags(m_detailEventTable->item(i,2)->flags() & ~Qt::ItemIsEditable);
                }
                //m_detailEventTable->item(i,2)->setFlags(m_detailEventTable->item(i,2)->flags() & ~Qt::ItemIsEditable);
            }
            if(isNoData)
            {
                const_cast<MAssociation*>(association)->setPinMap(pinMap);
            }
...
//runable是否已被连接
bool PropertiesView::MView::hasRunableConnected(const MAssociation *association, const QString& runableName)
{
    ModelController * modelController = m_propertiesView->modelController();
    MElement* elementB = modelController->findElement(association->endBUid());
    QList<MRelation *> relationList = modelController->findRelationsOfObject(dynamic_cast<MObject *>(elementB));
    for(MRelation * relation : relationList)
    {
        ...
        if(ass->endB().kind() == association->endB().kind())
        {
...
void PropertiesView::MView::prepare()
{
    ...
    if (!m_topWidget) {
        ...
        m_topLayout = new QVBoxLayout(m_topWidget);
        m_topWidget->setLayout(m_topLayout);
    }
    if (!m_referanceWidget) {
        m_referanceWidget = new QWidget();
        m_referanceLayout = new QFormLayout(m_referanceWidget);
        m_referanceWidget->setLayout(m_referanceLayout);
        m_topLayout->addWidget(m_referanceWidget);
    }
    if (!m_classNameLabel) {
        m_classNameLabel = new QLabel();
        m_referanceLayout->addRow(m_classNameLabel);
...
void PropertiesView::MView::insertRow(const char *before_id, const QString &label, QLayout *layout, const char *id)
{
    for (int i = m_rowToId.size() - 1; i >= 0; --i) {
        if (strcmp(m_rowToId.at(i), before_id) == 0) {
            m_referanceLayout->insertRow(i, label, layout);
...
void PropertiesView::MView::insertRow(const char *before_id, const QString &label, QWidget *widget, const char *id)
{
    for (int i = m_rowToId.size() - 1; i >= 0; --i) {
        if (strcmp(m_rowToId.at(i), before_id) == 0) {
            m_referanceLayout->insertRow(i, label, widget);
            m_rowToId.insert(i, id);
            return;
        }
    }
...
void PropertiesView::MView::insertRow(const char *before_id, QWidget *widget, const char *id)
{
    for (int i = m_rowToId.size() - 1; i >= 0; --i) {
        if (strcmp(m_rowToId.at(i), before_id) == 0) {
            m_referanceLayout->insertRow(i, widget);
            m_rowToId.insert(i, id);
            return;
        }
    }
...
QMap<int, int> PropertiesView::MView::index2Protocol()
{
    QMap<int, int> m;
    m.insert(0 , 1);
    m.insert(1 , 4);
...
QMap<int, int> PropertiesView::MView::protocol2Index()
{
    QMap<int, int> m;
    m.insert(1 , 0);
    m.insert(4 , 1);
...
QMap<int, int> PropertiesView::MView::index2Type()
{
    QMap<int, int> m;
    m.insert(0 , 2);
    m.insert(1 , 4);
...
QMap<int, int> PropertiesView::MView::type2Index()
{
    QMap<int, int> m;
    m.insert(2 , 0);
    m.insert(4 , 1);
...
QString PropertiesView::MView::getAddrFromString(const QHash <QString,QString>& hash, const QString& str)
{
    return hash.contains(str) ? m_serviceHash.value(str).split(";")[0] : "127.0.0.1";
}

int PropertiesView::MView::getPortFromString(const QHash <QString,QString>& hash, const QString& str)
{
    return (hash.contains(str) ? m_serviceHash.value(str).split(";")[1] : "42256").toInt();
}

int PropertiesView::MView::getProtocolFromString(const QHash <QString,QString>& hash, const QString& str)
{
    return (hash.contains(str) ? m_serviceHash.value(str).split(";")[2] : "1").toInt();
}

int PropertiesView::MView::getTypeFromString(const QHash <QString,QString>& hash, const QString& str)
{
    return (hash.contains(str) ? m_serviceHash.value(str).split(";")[3] : "2").toInt();
}

QString PropertiesView::MView::setServiceInfoToString(const QString& addr, int port, int protocol, int type)
{
    return QString("%1;%2;%3;%4").arg(addr).arg(port).arg(protocol).arg(type);
}
