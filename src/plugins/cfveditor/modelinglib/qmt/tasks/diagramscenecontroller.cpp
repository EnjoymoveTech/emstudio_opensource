...
{//haojie.fang 建立Association连接
    ...
    //haojie.fang add oid
    modelAssociation->setEndAOid(endAModelObject->oid());
    modelAssociation->setEndAName(endAModelObject->name());
    ...
    modelAssociation->setEndBOid(endBModelObject->oid());
    modelAssociation->setEndBName(endBModelObject->name());
...
void DiagramSceneController::createComInheritance(DComponent *derivedClass, DComponent *baseClass,
                                               const QList<QPointF> &intermediatePoints, MDiagram *diagram)
{
    //haojie.fang 建立Association连接
    m_diagramController->undoController()->beginMergeSequence(tr("Create Inheritance"));

    MComponent *derivedModelClass = m_modelController->findObject<MComponent>(derivedClass->modelUid());
    QMT_ASSERT(derivedModelClass, return);
    MComponent *baseModelClass = m_modelController->findObject<MComponent>(baseClass->modelUid());
    QMT_ASSERT(baseModelClass, return);
...
void DiagramSceneController::createComAssociation(DComponent *endAClass, DComponent *endBClass,
                                               const QList<QPointF> &intermediatePoints, MDiagram *diagram,
                                               std::function<void (MAssociation*, DAssociation*)> custom)
{//haojie.fang 建立Association连接
    m_diagramController->undoController()->beginMergeSequence(tr("Create Association"));

    MComponent *endAModelObject = m_modelController->findObject<MComponent>(endAClass->modelUid());
    QMT_ASSERT(endAModelObject, return);
    MComponent *endBModelObject = m_modelController->findObject<MComponent>(endBClass->modelUid());
    QMT_ASSERT(endBModelObject, return);
    ...
    auto modelAssociation = new MAssociation();
    modelAssociation->setEndAUid(endAModelObject->uid());
    modelAssociation->setEndAOid(endAModelObject->oid());//haojie.fang add oid
    modelAssociation->setEndAName(endAModelObject->name());
    MAssociationEnd endA = modelAssociation->endA();
    endA.setNavigable(true);
    modelAssociation->setEndA(endA);
...
void DiagramSceneController::dropNewElement(const QString &newElementId, const QString &name, const QString &stereotype, const QString &oid,
                                            ...
                                            const QPoint &viewPos, const QSize &viewSize)
{
    if (newElementId == QLatin1String(ELEMENT_TYPE_ANNOTATION)) {
    ...
    }else if (newElementId == QLatin1String(ELEMENT_TYPE_DEVICE)) {
        auto device = new DDevice();
        device->setPos(pos);
        device->setIp("127.0.0.1");
        device->setEcuID(initEcuid());

        m_diagramController->addElement(device, diagram);
        alignOnRaster(device, diagram);
        emit newElementCreated(device, diagram);
    }else if (newElementId == QLatin1String(ELEMENT_TYPE_BOUNDARY)) {
        auto boundary = new DBoundary();
        boundary->setPos(pos);

        //haojie.fang 增加基础服务配置
        QHash<QString, QString> baseSrvInfo;
        QList<QString> baseSrvSelect;
        QMapIterator<QString, EmosTools::tSrvFixedModule> modules(EmosTools::SrvParser::getInstance()->getModules());
        while (modules.hasNext()) {
            modules.next();

            if(!modules.value().show)
                continue;

            if(modules.value().checked)
                baseSrvSelect << modules.value().moduleName;

            QMapIterator<QString, EmosTools::tSrvProperty> property(modules.value().propertyMap);
            while (property.hasNext()) {
                ...
                if(property.value().propertyName.toLower().indexOf("port") >= 0)
                    baseSrvInfo.insert(modules.value().moduleName + ";" + property.value().propertyName,QString::number(initServicePort()));
                else
                    baseSrvInfo.insert(modules.value().moduleName + ";" + property.value().propertyName,property.value().dvalue);
            }
        }
        boundary->setBaseSrvInfo(baseSrvInfo);
        boundary->setBaseSrvSelect(baseSrvSelect);
        ...
        if (newElementId == QLatin1String(ELEMENT_TYPE_PACKAGE)) {
            ...
            if (!oid.isEmpty())//haojie.fang add oid
                component->setOid(oid);

            //haojie.fang add init
            component->setServiceInfo(initServiceInfo(oid));
            component->setPropertyInfo(QHash<QString,QString>());
            newObject = component;
        ...
        if (newObject) {
            newObject->setName(Utils::makeUniquelyNumberedInsensitive(name, getComAliasName(), QString("")));
                                                 ...
                                                 MDiagram *diagram)
{
    ...
    DElement *element = addObject(modelObject, pos, diagram);//haojie.fang 添加对象
...
QList<QPointF> DiagramSceneController::calConnectPoints(qmt::DComponent *A, qmt::DComponent *B, MAssociationEnd::Kind type)
{
    ...
    qreal ax = A->pos().x();
    qreal ay = A->pos().y();
    qreal aw = A->rect().width();
    qreal ah = A->rect().height();
    qreal bx = B->pos().x();
    qreal by = B->pos().y();
    qreal bw = B->rect().width();
    qreal bh = B->rect().height();
    qreal maxw = qMax(aw, bw);
    qreal maxh = qMax(ah, bh);
    qreal minw = qMin(aw, bw);
    qreal minh = qMin(ah, bh);

    auto getShift = [](MAssociationEnd::Kind asskind, qreal length) -> qreal{
        switch(asskind)
        {
    ...
    auto calStrategy1 = [&](){
        if(((pA - pB).x() >= 0 && (pA - pB).y() >= 0)  || ((pA - pB).x() <= 0 && (pA - pB).y() <= 0)) //B在A左上角 B在A右下角
        {
            intermediatePoints << QPointF((ax + bx)/2 + -1*getShift(type, minh), ay + getShift(type, minh))
                               << QPointF((ax + bx)/2 + -1*getShift(type, minh), by + getShift(type, minh));
        }
        else if(((pA - pB).x() <= 0 && (pA - pB).y() >= 0) || ((pA - pB).x() >= 0 && (pA - pB).y() <= 0)) //B在A右上角 B在A左下角
        {
            intermediatePoints << QPointF((ax + bx)/2 + getShift(type, minh), ay + getShift(type, minh))
                               << QPointF((ax + bx)/2 + getShift(type, minh), by + getShift(type, minh));
        }
    };
    auto calStrategy2 = [&](){
        if(((pA - pB).x() >= 0 && (pA - pB).y() >= 0)  || ((pA - pB).x() <= 0 && (pA - pB).y() <= 0)) //B在A左上角 B在A右下角
        {
            intermediatePoints << QPointF(bx + -1*getShift(type, minh), ay + getShift(type, minh));
        }
        else if(((pA - pB).x() <= 0 && (pA - pB).y() >= 0) || ((pA - pB).x() >= 0 && (pA - pB).y() <= 0)) //B在A右上角 B在A左下角
        {
            intermediatePoints << QPointF(bx + getShift(type, minh), ay + getShift(type, minh));
        }
    };
    auto calStrategy3 = [&](){
        if(qAbs((pA - pB).x()) >= (aw + bw) / 2) //B在A左右
        {
            intermediatePoints << QPointF((ax + bx) / 2, ah < bh ? ay + getShift(type, minh) : by + getShift(type, minh));
        }
        else if(qAbs((pA - pB).y()) >= (ah + bh) / 2) //B在A上下
        {
            intermediatePoints << QPointF(aw < bw ? ax + getShift(type, minh) : bx + getShift(type, minh), (ay + by) / 2);
        }
    };

    if(qAbs(ax-bx) > (aw + bw) / 2 + minh && qAbs(ay-by) > (ah + bh) / 2 + minh) // 两者距离较大时
        calStrategy1();
    else if(qAbs(ax-bx) > (aw + bw) / 2 - minh /2 && qAbs(ay-by) > (ah + bh) / 2 - minh /2) // 两者距离接近，但没有多少重叠距离时
        calStrategy2();
    else
        calStrategy3();
...
QSet<int> DiagramSceneController::getAllPort()
{
    m_portList.clear();
    for(MDiagram * diagram : m_diagramController->allDiagrams())
    {
        for(const auto& element : diagram->diagramElements())
        {
            if(dynamic_cast<DComponent*>(element))
            {
                ...
                while (i.hasNext()) {
                    i.next();
                    m_portList << i.value().split(";")[1].toInt();
                }

                QHashIterator<QString, QString> pinIt(dynamic_cast<DComponent*>(element)->pinInfo());
                while (pinIt.hasNext()) {
                    pinIt.next();
                    m_portList << pinIt.value().toInt();
                }
            }
            else if(dynamic_cast<DBoundary*>(element))
            {
                ...
                while (i.hasNext()) {
                    i.next();
                    m_portList << i.value().toInt();
                }
                i = dynamic_cast<DBoundary*>(element)->staticInfo();
                while (i.hasNext()) {
                    i.next();
                    m_portList << i.value().split(";").at(0).toInt();
                }
            }
        }
    }
...
QSet<QString> DiagramSceneController::getComAliasName()
{
    m_aliasNameList.clear();
    for(MDiagram * diagram : m_diagramController->allDiagrams())
    {
        for(const auto& element : diagram->diagramElements())
        {
            if(dynamic_cast<DComponent*>(element))
            {
                m_aliasNameList << dynamic_cast<DComponent*>(element)->name();
            }
        }
    }
    return m_aliasNameList;
}

QHash<QString, QString> DiagramSceneController::initServiceInfo(const QString& oid)
{
    QHash<QString, QString> serviceHash;
    EmosTools::IDLStruct idl = EmosTools::IdlParser::getIdlStruct(oid);
    if(idl.service.size() == 0)
        return serviceHash;

    getAllPort();

    QMapIterator<QString, int> i(idl.service);
    while (i.hasNext()) {
        i.next();
        int id = Utils::makeUniquelyId(m_portList, 1, COMPONENT_START_PORT);
        serviceHash.insert(i.key(), QString("127.0.0.1;%1;1;2").arg(id));
        m_portList << id;
    }
...
int DiagramSceneController::initServicePort()
{
    getAllPort();

    int port = Utils::makeUniquelyId(m_portList, 1, COMPONENT_START_PORT);
    m_portList << port;

    return port;
}

int DiagramSceneController::initEcuid()
{
    int ecuid = 1;
    ...
    for(MDiagram * diagram : m_diagramController->allDiagrams())
    {
        for(const auto& element : diagram->diagramElements())
        {
            DDevice* d =  dynamic_cast<DDevice*>(element);
            if(d)
            {
                m_ecuidList << d->ecuID();
            }
    ...
    ecuid = Utils::makeUniquelyId(m_ecuidList, 1, 1);
    m_ecuidList << ecuid;
...
DObject *DiagramSceneController::addObject(MObject *modelObject, const QPointF &pos, MDiagram *diagram)
{
    ...
    m_diagramController->addElement(diagramObject, diagram);//add
                                               ...
                                               MDiagram *diagram)
{
    ...
    diagramRelation->setEndAOid(endADiagramObject->oid());//haojie.fang add oid
    diagramRelation->setEndAName(endADiagramObject->name());
    ...
    diagramRelation->setEndBOid(endBDiagramObject->oid());
    diagramRelation->setEndBName(endBDiagramObject->name());
