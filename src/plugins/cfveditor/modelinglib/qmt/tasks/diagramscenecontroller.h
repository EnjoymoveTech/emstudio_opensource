...
#define COMPONENT_START_PORT 42256
...
public:
    ...
    void createComInheritance(DComponent *derivedClass, DComponent *baseClass,
                           const QList<QPointF> &intermediatePoints, MDiagram *diagram);
    void createComAssociation(DComponent *endAClass, DComponent *endBClass,
                           const QList<QPointF> &intermediatePoints, MDiagram *diagram,
                           std::function<void (MAssociation*, DAssociation*)> custom = nullptr);
    ...
    void dropNewElement(const QString &newElementId, const QString &name, const QString &stereotype,const QString &oid,
    ...
    static QList<QPointF> calConnectPoints(DComponent *A, DComponent *B, MAssociationEnd::Kind type);

    QSet<int> getAllPort();
    QSet<QString> getComAliasName();
private:

    QHash<QString, QString> initServiceInfo(const QString& oid);
    int initServicePort();
    int initEcuid();
    ...
    QSet<int> m_portList;
    QSet<int> m_ecuidList;
    QSet<QString> m_aliasNameList;
};
