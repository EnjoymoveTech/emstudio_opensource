...
class DeviceStyleKey
{
};

uint qHash(const DeviceStyleKey &styleKey)
{
    Q_UNUSED(styleKey)
...
bool operator==(const DeviceStyleKey &lhs, const DeviceStyleKey &rhs)
{
    Q_UNUSED(lhs)
    Q_UNUSED(rhs)
...
const Style *DefaultStyleEngine::applyDeviceStyle(const Style *baseStyle, const DDevice *boundary,
                                                    const Parameters *parameters)
{
    Q_UNUSED(boundary)

    return applyDeviceStyle(baseStyle, parameters);
}
...
const Style *DefaultStyleEngine::applyDeviceStyle(const Style *baseStyle, const StyleEngine::Parameters *parameters)
{
    Q_UNUSED(parameters)

    DeviceStyleKey key;
    const Style *derivedStyle = m_deviceStyleMap.value(key);
    if (!derivedStyle) {
        auto style = new Style(baseStyle->type());
        style->setNormalFont(baseStyle->normalFont());
        style->setTextBrush(baseStyle->textBrush());
        m_deviceStyleMap.insert(key, style);
        derivedStyle = style;
    }
    return derivedStyle;
}
