...
#include "methodstarterstyle.h"
#include "eventstarterstyle.h"
#include "fieldstarterstyle.h"
#include "pinstarterstyle.h"
...
StyleController::StyleController(QObject *parent)
    : QObject(parent),
      ...
      m_methodStarterStyle(new MethodStarterStyle),
      m_eventStarterStyle(new EventStarterStyle),
      m_fieldStarterStyle(new FieldStarterStyle),
      m_pinStarterStyle(new PinStarterStyle),
...
const Style *StyleController::adaptDeviceStyle(const DDevice *boundary)
{
    Parameters parameters(this);
    return m_defaultStyleEngine->applyDeviceStyle(m_defaultStyle.data(), boundary, &parameters);
}
...
const Style *StyleController::methodStarterStyle()
{
    return m_methodStarterStyle.data();
}

const Style *StyleController::eventStarterStyle()
{
    return m_eventStarterStyle.data();
}

const Style *StyleController::fieldStarterStyle()
{
    return m_fieldStarterStyle.data();
}

const Style *StyleController::pinStarterStyle()
{
    return m_pinStarterStyle.data();
}
