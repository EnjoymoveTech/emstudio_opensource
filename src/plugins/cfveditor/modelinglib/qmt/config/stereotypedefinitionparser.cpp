...
//haojie.fang add Relation type

static const int KEYWORD_METHOD       = 120;
static const int KEYWORD_EVENT        = 121;
static const int KEYWORD_FIELD        = 122;
static const int KEYWORD_PIN          = 123;
...
void StereotypeDefinitionParser::parse(ITextSource *source)
{
    ...
    textScanner.setKeywords(
                ...
                << qMakePair(QString("method"), KEYWORD_METHOD)
                << qMakePair(QString("event"), KEYWORD_EVENT)
                << qMakePair(QString("field"), KEYWORD_FIELD)
                << qMakePair(QString("pin"), KEYWORD_PIN)
...
void StereotypeDefinitionParser::parseFile()
{
    for (;;) {
        ...
        if (token.type() == Token::TokenEndOfInput)
            ...
            throw StereotypeDefinitionParserError("Expected 'Icon', 'Toolbar', 'Relation', 'Dependency', 'Method', 'Event' , 'Field', 'Thread', Inheritance' or 'Association'.", token.sourcePos());
    }
}
    ...
    while (readProperty(&token)) {
        ...
        case KEYWORD_RELATIONSHIP:
        {
            ...
            const static QHash<QString, CustomRelation::Relationship> relationshipNames = QHash<QString, CustomRelation::Relationship>()
                    ...
                    << qMakePair(QString("method"), CustomRelation::Relationship::Method)
                    << qMakePair(QString("event"), CustomRelation::Relationship::Event)
                    << qMakePair(QString("field"), CustomRelation::Relationship::Field)
                    << qMakePair(QString("pin"), CustomRelation::Relationship::Pin);
