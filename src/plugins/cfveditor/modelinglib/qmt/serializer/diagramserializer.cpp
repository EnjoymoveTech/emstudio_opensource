...
inline void Access<Archive, DElement>::serialize(Archive &archive, DElement &element)
{
    archive || tag(element)
            ...
            || attr("oid", element, &DElement::oid, &DElement::setOid)//haojie.fang add oid
            || end;
}
...
inline void Access<Archive, DComponent>::serialize(Archive &archive, DComponent &component)
{
    archive || tag(component)
            ...
            || attr("plain-shape", component, &DComponent::isPlainShape, &DComponent::setPlainShape)//haojie.fang add service
            || attr("serviceInfo", component, &DComponent::serviceInfo, &DComponent::setServiceInfo)
            || attr("propertyInfo", component, &DComponent::propertyInfo, &DComponent::setPropertyInfo)
            || attr("pinInfo", component, &DComponent::pinInfo, &DComponent::setPinInfo)
            || end;
}
...
inline void Access<Archive, DRelation>::serialize(Archive &archive, DRelation &relation)
{
    archive || tag(relation)
            ...
            || attr("a_oid", relation, &DRelation::endAOid, &DRelation::setEndAOid)//haojie.fang add oid
            || attr("a_name", relation, &DRelation::endAName, &DRelation::setEndAName)//
            || attr("b", relation, &DRelation::endBUid, &DRelation::setEndBUid)
            || attr("b_oid", relation, &DRelation::endBOid, &DRelation::setEndBOid)
            || attr("b_name", relation, &DRelation::endBName, &DRelation::setEndBName)
...
inline void Access<Archive, DAssociationEnd>::serialize(Archive &archive, DAssociationEnd &associationEnd)
{
    archive || tag(associationEnd)
            ...
            || attr("kindName", associationEnd, &DAssociationEnd::kindName, &DAssociationEnd::setKindName)
            || end;
}
...
inline void Access<Archive, DAssociation>::serialize(Archive &archive, DAssociation &association)
{
    archive || tag(association)
            ...
            || attr("eventMap", association, &DAssociation::getEventMap, &DAssociation::setEventMap)
            || attr("fieldMap", association, &DAssociation::getFieldMap, &DAssociation::setFieldMap)
            || attr("pinMap", association, &DAssociation::getPinMap, &DAssociation::setPinMap)
            || end;
}
...
inline void Access<Archive, DBoundary>::serialize(Archive &archive, DBoundary &boundary)
{
    archive || tag(boundary)
            ...
            || attr("baseSrvInfo", boundary, &DBoundary::baseSrvInfo, &DBoundary::setBaseSrvInfo)
            || attr("baseSrvSelect", boundary, &DBoundary::baseSrvSelect, &DBoundary::setBaseSrvSelect)
            || attr("staticSelected", boundary, &DBoundary::staticSelected, &DBoundary::setStaticSelected)
            || attr("staticInfo", boundary, &DBoundary::staticInfo, &DBoundary::setStaticInfo)
            || end;
}
...
QARK_REGISTER_TYPE_NAME(DDevice, "DDevice")
QARK_REGISTER_DERIVED_CLASS(QXmlInArchive, QXmlOutArchive, DDevice, DElement)
QARK_ACCESS_SERIALIZE(DDevice)

template<class Archive>
inline void Access<Archive, DDevice>::serialize(Archive &archive, DDevice &boundary)
{
    archive || tag(boundary)
            || base<DElement>(boundary)
            || attr("text", boundary, &DDevice::text, &DDevice::setText)
            || attr("pos", boundary, &DDevice::pos, &DDevice::setPos)
            || attr("rect", boundary, &DDevice::rect, &DDevice::setRect)
            || attr("ip", boundary, &DDevice::ip, &DDevice::setIp)
            || attr("ecuID", boundary, &DDevice::ecuID, &DDevice::setEcuID)
            || end;
}

QARK_ACCESS_SPECIALIZE(QXmlInArchive, QXmlOutArchive, DDevice)
