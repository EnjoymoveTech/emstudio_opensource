...
public:
    ...
    QByteArray leftGroupSplitterState;
};
...
bool UiController::hasLeftGroupSplitterState() const
{
    return !d->leftGroupSplitterState.isEmpty();
}

QByteArray UiController::leftGroupSplitterState() const
{
    return d->leftGroupSplitterState;
}
...
void UiController::onLeftGroupSplitterChanged(const QByteArray &state)
{
    d->leftGroupSplitterState = state;
    emit leftGroupSplitterChanged(state);
}
...
{
    if (hasLeftGroupSplitterState()) {
        settings->setValue(
                    QLatin1String(Constants::SETTINGS_LEFT_GROUP_SPLITTER), d->leftGroupSplitterState);
    }
...
{
    if (settings->contains(QLatin1String(Constants::SETTINGS_LEFT_GROUP_SPLITTER))) {
        d->leftGroupSplitterState = settings->value(
                    QLatin1String(Constants::SETTINGS_LEFT_GROUP_SPLITTER)).toByteArray();
    }
