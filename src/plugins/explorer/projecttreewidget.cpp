...
ProjectTreeWidget::ProjectTreeWidget(QWidget *parent) : QWidget(parent)
{
    ...
    m_filterProjectsAction->setChecked(true); // default is the traditional complex tree haojie.fang 默认简易视图
    ...
    m_trimEmptyDirectoriesAction->setChecked(false);//haojie.fang 默认不隐藏
...
void ProjectTreeWidget::editCurrentItem()
{
    ...
    QMap<int, QVariant> map = m_model->itemData(currentIndex);
    QString name = map.value(0).toString();
    if(name.endsWith(".description")) // description files cannot rename!
    {
        EmosWidgets::MessageTips::showText(this->parentWidget(), "can't rename!");
        return;
    }
...
void ProjectTreeWidgetFactory::restoreSettings(QSettings *settings, int position, QWidget *widget)
{
    ...
    Q_ASSERT(ptw);
    const QString baseKey = QLatin1String("ProjectTreeWidget.") + QString::number(position);
    ptw->setProjectFilter(settings->value(baseKey + QLatin1String(".ProjectFilter"), true).toBool());//haojie.fang 默认简易模式
    ...
    ptw->setTrimEmptyDirectories(settings->value(baseKey + QLatin1String(".TrimEmptyDirsFilter"), false).toBool());//haojie.fang 默认不隐藏
