...
#include <generaleditor/fontsettings.h>
#include <generaleditor/texteditorsettings.h>

#include <pluginmanager/invoker.h>
...
static Q_LOGGING_CATEGORY(appOutputLog, "qtc.explorer.appoutput", QtWarningMsg);
...
void AppOutputPane::appendMessage(RunControl *rc, const QString &out, Utils::OutputFormat format)
{
    ...
    if (index != -1) {
        ...
        if (format == Utils::NormalMessageFormat || format == Utils::ErrorMessageFormat || format == Utils::WarningMessageFormat) {
