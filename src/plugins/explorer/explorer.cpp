...
#include <pluginmanager/pluginspec.h>
#include <ssh/sshconnection.h>
#include <ssh/sshsettings.h>
#include <generaleditor/findinfiles.h>
#include <generaleditor/textdocument.h>
#include <generaleditor/texteditorconstants.h>

#include <explorer/coderootselector.h>
...
const char CODEROOT[]             = "ProjectExplorer.CodeRoot";
...
const int  P_ACTION_CODEROOT       = 60;
...
public:
    ...
    QAction *m_coderootAction;//CodeRoot
    ...
    CodeRootSelector * m_coderootSelector;
    ...
    //ProjectsMode m_projectsMode; haojie.fang
    ...
    //zhecheng.zhou<Application Output>
    ...
    FolderNavigationWidgetFactory m_folderNavigationWidgetFactory;  //haojie.fang 去掉文件系统选择
...
//    mdebug->addMenu(mstartdebugging, Core::Constants::G_DEFAULT_ONE);
    ...
    mfileContextMenu->appendGroup(Constants::G_FILE_BUILD);
    ...
    //ModeManager::addAction(dd->m_modeBarBuildAction, Constants::P_ACTION_BUILDPROJECT);  //haojie.fang 去掉mode action
    ...
    //ModeManager::addAction(cmd->action(), Constants::P_ACTION_RUN);  //haojie.fang 去掉mode action
...
//    mproject->addAction(cmd, Constants::G_FOLDER_FILES);
    ...
    //haojie.fang select CodeRoot
    dd->m_coderootAction = new QAction("CodeRoot", this);
    dd->m_coderootAction->setObjectName("CodeRoot"); // used for UI introduction
    dd->m_coderootAction->setCheckable(true);
    dd->m_coderootAction->setEnabled(false);
    dd->m_coderootSelector = new CodeRootSelector(dd->m_coderootAction, ICore::dialogParent());
    connect(dd->m_coderootAction, &QAction::triggered,
            dd->m_coderootSelector, &QWidget::show);
    ModeManager::addAction(dd->m_coderootAction, Constants::P_ACTION_CODEROOT);  //haojie.fang 去掉mode action
    ...
    dd->m_projectExplorerSettings.saveBeforeBuild =
            s->value(Constants::SAVE_BEFORE_BUILD_SETTINGS_KEY, true).toBool();
...
void ProjectExplorerPluginPrivate::updateActions()
{
    ...
    m_coderootAction->setEnabled(hasProjects);
    ...
    //haojie.fang不显示
    m_projectSelectorActionMenu->setVisible(false);
    m_buildProjectOnlyAction->setVisible(false);
    m_buildProjectForAllConfigsAction->setVisible(false);
    m_buildAction->setVisible(false);
    m_buildForRunConfigAction->setVisible(false);

    m_deployProjectOnlyAction->setVisible(false);
    m_deployAction->setVisible(false);
    m_deployActionContextMenu->setVisible(false);
    m_deploySessionAction->setVisible(false);

    m_buildActionContextMenu->setVisible(false);
    m_buildDependenciesActionContextMenu->setVisible(false);
    m_buildSessionAction->setVisible(false);
    m_buildSessionForAllConfigsAction->setVisible(false);
    m_rebuildProjectOnlyAction->setVisible(false);
    m_rebuildAction->setVisible(false);
    m_rebuildProjectForAllConfigsAction->setVisible(false);
    m_rebuildActionContextMenu->setVisible(false);
    m_rebuildDependenciesActionContextMenu->setVisible(false);
    m_rebuildSessionAction->setVisible(false);
    m_rebuildSessionForAllConfigsAction->setVisible(false);
    m_cleanProjectOnlyAction->setVisible(false);

    m_cleanAction->setVisible(false);
    m_cleanProjectForAllConfigsAction->setVisible(false);
    m_cleanActionContextMenu->setVisible(false);
    m_cleanDependenciesActionContextMenu->setVisible(false);
    m_cleanSessionAction->setVisible(false);
    m_cleanSessionForAllConfigsAction->setVisible(false);
    m_runAction->setVisible(false);
    m_runActionContextMenu->setVisible(false);
    m_runWithoutDeployAction->setVisible(false);
    m_cancelBuildAction->setVisible(false);
...
void ProjectExplorerPlugin::selectCodeRoot()
{
    //activeBuildConfiguration()->setBuildDirectory();
}
...
//        return;
    ...
    Core::FileUtils::removeFiles(pathList, true);
}
...
void ProjectExplorerPlugin::openOpenProjectDialog()
{
    const QString path = DocumentManager::useProjectsDirectory()
                             ...
                             : QString();
    const QStringList files = DocumentManager::getOpenFileNames(QString(), path);
    if (!files.isEmpty())
