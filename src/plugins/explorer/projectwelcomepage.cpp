...
static QFont sizedFont(int size, const QWidget *widget, bool underline = false, bool bold = false)
{
    QFont f = widget->font();
    f.setPixelSize(size);
...
public:
    void paint(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &idx) const final
    {
        ...
        const int x = rc.x();
        const int y = rc.y();
        const int firstBase = y + 16;
        const int secondBase = firstBase + 16;
        ...
        //if (idx.row() < 9)
            //painter->drawText(x + 3, firstBase, QString::number(idx.row() + 1));
            painter->drawText(x + 3, firstBase, "*");

        const int textSpace = rc.width() - 20 - 6;

        painter->setPen(themeColor(Theme::Welcome_LinkColor));
        painter->setFont(sizedFont(13, option.widget, hovered, true));
        const QString projectNameElided =
                        painter->fontMetrics().elidedText(projectName, Qt::ElideRight, textSpace);
        painter->drawText(x + 20, firstBase, projectNameElided);
        ...
        painter->drawText(x + 20, secondBase, pathWithTildeElided);
    }
    ...
    QSize sizeHint(const QStyleOptionViewItem &option, const QModelIndex &idx) const final
    {
        ...
        int width = std::max(fm.horizontalAdvance(projectName),
                             fm.horizontalAdvance(projectPath)) + 20;
        ...
        if (ev->type() == QEvent::MouseButtonRelease) {
            ...
            if (button == Qt::RightButton) {
                ...
                QAction *action = new QAction(
                    ProjectWelcomePage::tr("Remove Recent Project"));
                ...
                action = new QAction(ProjectWelcomePage::tr("Clear Recent Projects"));
...
//        newButton->setOnClicked([] { ProjectExplorerPlugin::openNewProjectDialog(); });
        ...
        EmosWidgets::FlatButton* flatbuttonNew = new EmosWidgets::FlatButton("New", this);
        flatbuttonNew->setBackgroundColor(EmosWidgets::WidgetsStyle::instance().themeColor(EmosWidgets::blue400));
        QColor backColor = EmosWidgets::WidgetsStyle::instance().themeColor(EmosWidgets::pink500);
        flatbuttonNew->setPressBackgroundColor(backColor);
        backColor.setAlpha(150);
        flatbuttonNew->setHoverBackgroundColor(backColor);
        flatbuttonNew->setFixedWidth(100);
        connect(flatbuttonNew, &EmosWidgets::FlatButton::clicked, [](){ ProjectExplorerPlugin::openNewProjectDialog(); });

        EmosWidgets::FlatButton* flatbuttonOpen = new EmosWidgets::FlatButton("Open", this);
        flatbuttonOpen->setBackgroundColor(EmosWidgets::WidgetsStyle::instance().themeColor(EmosWidgets::blue400));
        backColor.setAlpha(255);
        flatbuttonOpen->setPressBackgroundColor(backColor);
        backColor.setAlpha(150);
        flatbuttonOpen->setHoverBackgroundColor(backColor);
        flatbuttonOpen->setFixedWidth(100);
        connect(flatbuttonOpen, &EmosWidgets::FlatButton::clicked, [](){ ProjectExplorerPlugin::openOpenProjectDialog(); });
...
//        hbox11->addStretch(1);
        ...
        hbox21->addWidget(flatbuttonNew);
        hbox21->addSpacing(16);
        hbox21->addWidget(flatbuttonOpen);
        hbox21->addStretch(1);
