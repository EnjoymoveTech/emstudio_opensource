...
UserFileAccessor::UserFileAccessor(Project *project) :
    MergingSettingsAccessor(std::make_unique<VersionedBackUpStrategy>(this),
                            "EmStudioProject", project->displayName(),
...
FilePath UserFileAccessor::projectUserFile() const
{
    ...
    Utils::FilePath projectPath = m_project->projectFilePath();

    //haojie.fang user路径修改到.emos下
    return projectPath.parentDir().pathAppended(".emos").pathAppended(projectPath.fileName())
