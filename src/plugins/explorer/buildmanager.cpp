...
{
    return Internal::CompileOutputSettings();
...
void BuildManager::flushOutputWindow()
{
    //d->m_outputWindow->flush();
    QApplication::alert(ICore::dialogParent(), 3000);
}
...
void BuildManager::popupOutputWindow()
{
    //d->m_outputWindow->popup(IOutputPane::ModeSwitch);
}
                                     ...
                                     BuildStep::OutputNewlineSetting newlineSettings)
{
    ...
    if (format == BuildStep::OutputFormat::NormalMessage || format == BuildStep::OutputFormat::ErrorMessage || format == BuildStep::OutputFormat::WarningMessage)
    {
