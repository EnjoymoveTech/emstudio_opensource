...
void KitManager::restoreKits()
{
    ...
    d->m_writer = std::make_unique<PersistentSettingsWriter>(settingsFileName(), "EmStudioProfiles");
...
{
    //haojie.fang 创建一个假的kit
    KitList result;
    Utils::Id id_self = Utils::Id::fromString("c3a53e12-59fd-4dc3-afbd-134699944760");
    auto k = std::make_unique<Kit>(id_self);
    result.kits.emplace_back(std::move(k));
    result.defaultKit = id_self;
    return result;
}
