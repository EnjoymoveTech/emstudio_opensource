...
private:
    ...
    bool userWantsContextMenu(const QMouseEvent *e) const
    {
        ...
        // causing unwanted kit activation (EMSTUDIOBUG-24156). Let's suppress these.
...
public:
    ...
    void registerProject(Project *project)
    {
        ...
        auto projectItem = new ProjectItem(project, [this] {
            //updatePanel();
        });
    ...
    void startupProjectChanged(Project *project)
    {
        ...
        //haojie.fang 默认添加kits
        if (auto factory = BuildConfigurationFactory::find(KitManager::kits().first(), project->projectFilePath()))
        {
            project->setup(factory->allAvailableSetups(KitManager::kits().first(), project->projectFilePath()));
        }
