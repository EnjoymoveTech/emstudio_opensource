        ...
        bool expandWildcards) const
{
    ...
    foreach (const ProString &str, d->values(ProKey(variable))) {
        ...
        if (expandWildcards) {
            ...
            expandWildcards = fileNamePattern.toString().contains('*') || fileNamePattern.toString().contains('?');
        }
        if (expandWildcards) {
...
int ProFileEvaluator::versionValue() const
{
    const ProStringList &templ = d->values(ProKey("VERSION"));
    return templ.count() >= 1 ? templ.at(0).toInt() : -1;
}
