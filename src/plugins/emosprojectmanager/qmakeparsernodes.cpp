...
#include <explorer/editorconfiguration.h>
#include <explorer/explorer.h>
#include <explorer/projectexplorerconstants.h>
#include <explorer/target.h>
#include <explorer/taskhub.h>
#include <generaleditor/icodestylepreferences.h>
#include <generaleditor/tabsettings.h>
#include <generaleditor/texteditorsettings.h>
...
public:
    ...
    FilePath sysroot;
    ProFileReader *readerExact;
    ProFileReader *readerCumulative;
...
QStringList QmakePriFile::baseVPaths(ProFileReader *reader, const QString &projectDir, const QString &buildDir)
{
    QStringList result;
    if (!reader)
        return result;
...
QStringList QmakePriFile::fullVPaths(const QStringList &baseVPaths, ProFileReader *reader,
...
QStringList QmakePriFile::varNames(FileType type, ProFileReader *readerExact)
{
    QStringList vars;
    switch (type) {
    ...
    case FileType::Source: {
        vars << QLatin1String("SOURCES") << QLatin1String("CONTENTS");
...
QString QmakePriFile::varNameForAdding(const QString &mimeType)
{
    ...
    //haojie.fang add qmake mimeType
    if (mimeType == QLatin1String(ProjectExplorer::Constants::C_IDL_MIMETYPE)
               || mimeType == QLatin1String(ProjectExplorer::Constants::C_CFV_MIMETYPE))
    {
        return QLatin1String("CONTENTS");
    }
...
static bool evaluateOne(const QmakeEvalInput &input, ProFile *pro,
                        ProFileReader *reader, bool cumulative,
                        ProFileReader **buildPassReader)
{
    ...
    if (builds.isEmpty()) {
        *buildPassReader = reader;
        ...
        auto bpReader = new ProFileReader(input.qmakeGlobals, input.qmakeVfs); // needs to access m_qmakeGlobals, m_qmakeVfs
...
QmakeEvalResult *QmakeProFile::evaluate(const QmakeEvalInput &input)
{
    ...
    ProFileReader *exactBuildPassReader = nullptr;
    ProFileReader *cumulativeBuildPassReader = nullptr;
    ProFile *pro;
    ...
    if (result->state == QmakeEvalResult::EvalOk) {
        ...
        result->newVarValues[Variable::ExactSource] =
                ...
                fileListForVar(exactSourceFiles, QLatin1String("CONTENTS")) +
...
QString QmakeProFile::uiDirPath(ProFileReader *reader, const FilePath &buildDir)
{
...
QString QmakeProFile::mocDirPath(ProFileReader *reader, const FilePath &buildDir)
{
...
QStringList QmakeProFile::includePaths(ProFileReader *reader, const FilePath &sysroot,
                                           const FilePath &buildDir, const QString &projectDir)
{
    ...
    // unfixified value. This can be necessary for cross-building; see EMSTUDIOBUG-21164.
    if (tryUnfixified) {
...
QStringList QmakeProFile::libDirectories(ProFileReader *reader)
{
    QStringList result;
...
FilePaths QmakeProFile::subDirsPaths(ProFileReader *reader,
...
TargetInformation QmakeProFile::targetInformation(ProFileReader *reader,
                                                  ProFileReader *readerBuildPass,
...
InstallsList QmakeProFile::installsList(const ProFileReader *reader, const QString &projectFilePath,
