                                   ...
                                   const QVariantMap &extraVariables)
{
    ...
    QString dirName = defaultLocation.split("/").last();
    QString displayname = QString();
    if (dirName == "3_architecture_design")
    {
        displayname = "Cfv File";
    }
    else if (dirName == "2_component_design")
    {
        displayname = "Idl File";
    }
    else if (dirName == "schedule_configuration")
    {
        displayname = "Scheduler File";
    }
    for (IWizardFactory *factory : qAsConst(factories)) {
        ...
        if ((displayname == factory->displayName()) || displayname.isEmpty())
        {
            qDebug()<<factory->displayName();
...
void NewDialog::addItem(QStandardItem *topLevelCategoryItem, IWizardFactory *factory)
{
    ...
    if (!categoryItem) {
        ...
        QString d = factory->displayCategory();
