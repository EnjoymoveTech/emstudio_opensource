    ...
    , m_toggleRightSideBarButton(new QToolButton)
{
    ...
    if (HostOsInfo::isLinuxHost())
        QApplication::setWindowIcon(Icons::EMSTUDIOLOGO_BIG.icon());
...
void MainWindow::closeEvent(QCloseEvent *event)
{
    ...
    //haojie.fang 默认保存不提示
    if (!DocumentManager::saveAllModifiedDocumentsSilently()) {
        cancelClose();
        return;
    }
...
void MainWindow::registerDefaultActions()
{
    ...
    m_openAction = new QAction(icon, tr("&Open Project"), this);
    ...
    cmd = ActionManager::registerAction(tmpaction, Constants::ABOUT_EMSTUDIO);
    mhelp->addAction(cmd, Constants::G_HELP_ABOUT);
    tmpaction->setEnabled(true);
    connect(tmpaction, &QAction::triggered, this, &MainWindow::aboutEmStudio);
...
//    connect(tmpaction, &QAction::triggered, qApp, &QApplication::aboutQt);
    ...
    if (!HostOsInfo::isMacHost()) { // doesn't have the "About" actions in the Help menu
        ...
        cmd = ActionManager::registerAction(tmpaction, "EmStudio.Help.Sep.About");
        mhelp->addAction(cmd, Constants::G_HELP_ABOUT);
    }
}
...
void MainWindow::readSettings()
{
...

    {
        ModeManager::Style modeStyle =
                ModeManager::Style(settings->value(modeSelectorLayoutKey, int(ModeManager::Style::IconsOnly)).toInt());
        ...
        if (!settings->contains(modeSelectorLayoutKey) && settings->contains(modeSelectorVisibleKey)) {
            ...
            modeStyle = visible ? ModeManager::Style::IconsOnly : ModeManager::Style::Hidden;
        }
