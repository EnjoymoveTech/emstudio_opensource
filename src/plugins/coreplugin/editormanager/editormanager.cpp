...
/*!
    ...
    \l{https://doc.qt.io/emstudio/creator-coding-navigating.html#splitting-the-editor-view}
...
bool EditorManagerPrivate::closeEditors(const QList<IEditor*> &editors, CloseFlag flag)
{
    ...
    else //haojie.fang 默认保存修改
    {
        ...
        bool cancelled = false;
        QList<IDocument *> rejectedList;
        DocumentManager::saveModifiedDocumentsSilently(documentsToClose, &cancelled, &rejectedList);
    }
...
void EditorManager::setEmptyWidget(QWidget *w)
{
    if(w)
        EditorManagerPrivate::currentEditorView()->setEmptyWidget(w);
}
...
QStringList EditorManager::getOpenFileNames()
{
    ...
    return DocumentManager::getOpenFileNames(QString(), QString());
}
