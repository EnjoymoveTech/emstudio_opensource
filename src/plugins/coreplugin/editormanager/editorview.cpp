    ...
    m_statusWidget(new QFrame(this))
{
    ...
    empty->hide();
    m_emptyLayout = new QGridLayout(empty);
    m_emptyLayout->setContentsMargins(0,0,0,0);
    empty->setLayout(m_emptyLayout);
    ...
    //haojie.fang 增加自定义初始空页面
    //m_emptyLayout->addWidget(m_emptyViewLabel);
...
void EditorView::setEmptyWidget(QWidget *w)
{
    m_emptyLayout->addWidget(w);
}
