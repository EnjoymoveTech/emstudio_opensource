...
static Utils::optional<QIcon> getIcon(QHash<QString, QIcon> &cache, const QString &key)
{
    auto it = cache.constFind(key);
    if (it == cache.constEnd())
        return {};
    if (const QIcon *icon = &it.value())
        return *icon;
    // need to create icon from file name first
    const QIcon icon = QIcon(
        FileIconProvider::overlayIcon(QStyle::SP_FileIcon, QIcon(it.value().name()), QSize(16, 16)));
    //emos todo
...
public:
    ...
    {
        //emos todo
        m_filenameCache.insert(filename, QIcon(iconFilePath));
    }

    void registerIconOverlayForSuffix(const QString &iconFilePath, const QString &suffix)
    {
        //emos todo
        m_suffixCache.insert(suffix, QIcon(iconFilePath));
    }
    ...
    mutable QHash<QString, QIcon> m_filenameCache;
};
...
QIcon directoryIcon(const QString &overlay)
{
    ...
    //lingfei.yu change not add back icon
    return overlayIcon;
}
