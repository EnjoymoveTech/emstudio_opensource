...
/*!
    ...
    wizards, as instructed in \l{https://doc.qt.io/emstudio/creator-project-wizards.html}
...
Utils::Wizard *IWizardFactory::runWizard(const QString &path, QWidget *parent, Id platform, const QVariantMap &variables)
{
    ...
    if (wizard) {
        ...
        connect(wizard, &QObject::destroyed, this, []() {
            ...
            //s_inspectWizardAction->setEnabled(false);  //remove by haojie.fang
            ICore::updateNewItemDialogState();
            s_reopenData.reopen();
        });
        //s_inspectWizardAction->setEnabled(true);  //remove by haojie.fang
        wizard->show();
