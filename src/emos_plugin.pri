...
dependencyList =
for(dep, plugin_deps) {
    dependencyList += "        { \"Name\" : \"$$dependencyName($$dep)\", \"Version\" : \"$$EMSTUDIO_VERSION\" }"
}
for(dep, plugin_recmds) {
    dependencyList += "        { \"Name\" : \"$$dependencyName($$dep)\", \"Version\" : \"$$EMSTUDIO_VERSION\", \"Type\" : \"optional\" }"
}
for(dep, plugin_test_deps) {
    dependencyList += "        { \"Name\" : \"$$dependencyName($$dep)\", \"Version\" : \"$$EMSTUDIO_VERSION\", \"Type\" : \"test\" }"
}
...
isEmpty(PRECOMPILED_HEADER):PRECOMPILED_HEADER = $$PWD/shared/emstudio_gui_pch.h

isEmpty(USE_USER_DESTDIR) {
    DESTDIR = $$IDE_PLUGIN_PATH
} else {
    win32 {
        DESTDIRAPPNAME = "emstudio"
    ...
    DESTDIR = "$$DESTDIRBASE/EmStudio/$$DESTDIRAPPNAME/plugins/$$EMSTUDIO_VERSION"
}
LIBS += -L$$DESTDIR
INCLUDEPATH += $$OUT_PWD
...
isEmpty(TARGET) {
    error("emstudioplugin.pri: You must provide a TARGET")
}
...
osx {
    ...
    QMAKE_LFLAGS += -compatibility_version $$EMSTUDIO_COMPAT_VERSION
}
